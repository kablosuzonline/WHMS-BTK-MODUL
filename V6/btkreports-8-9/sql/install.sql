...
-- BTK tarafından sağlanan yetki türleri için referans tablosu
...
-- Kullanıcının sahip olduğu aktif yetki türlerini tutan tablo
...
-- BTK EK-3'te belirtilen hizmet tipleri için referans tablosu
...
-- WHMCS ürün grupları ile BTK yetki ve hizmet tiplerini eşleştiren tablo
...
-- Personel departmanları için referans tablosu
...
-- ISS POP Noktaları ve İzleme Yetenekleri
CREATE TABLE IF NOT EXISTS `mod_btk_iss_pop_noktalari` (
    ...
    `izleme_port` INT DEFAULT NULL COMMENT 'İleride kullanılacak izleme portu (örn: SNMP)',
...
-- Abone verilerinin anlık durumunu tutan "REHBER" tablosu
CREATE TABLE IF NOT EXISTS `mod_btk_abone_rehber` (
    ...
    `packageid` INT DEFAULT NULL COMMENT 'tblproducts.id, tarife değişikliğini takip etmek için',
...
-- Raporlanacak abone hareketlerini tutan "canlı" tablo
...
-- Gönderilmiş eski hareketlerin arşiv tablosu
...
-- FTP gönderimlerinin log tablosu
CREATE TABLE IF NOT EXISTS `mod_btk_ftp_logs` (
    ...
    `gonderim_zamani` TIMESTAMP(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
