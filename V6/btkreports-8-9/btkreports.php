 ...
 * Sürüm: 8.0.9 (Vendor İzolasyon Düzeltmesi)
 * @package    WHMCS
...
// --- GÜVENLİ YÜKLEME MEKANİZMASI (v8.0.9) ---
// Önce WHMCS'in ana autoloader'ında sınıfların olup olmadığını kontrol et
if (!class_exists('PhpOffice\PhpSpreadsheet\Spreadsheet') || !class_exists('Cron\CronExpression')) {
    // Eğer yoksa, sadece bizim modülümüze ait autoloader'ı çağır
    $btkAutoload = __DIR__ . '/vendor/autoload.php';
    if (file_exists($btkAutoload)) {
        require_once $btkAutoload;
    } else {
        // Bu kritik bir hata, modül çalışamaz.
        // Bu aşamada loglama yapamayabiliriz.
        die("BTK Raporlama Modülü için gerekli kütüphaneler bulunamadı. Lütfen modül dizininde 'composer install --no-dev' komutunu çalıştırın.");
    }
}
// Tekrar kontrol et, eğer hala yoksa bir sorun var demektir.
if (!class_exists('PhpOffice\PhpSpreadsheet\Spreadsheet') || !class_exists('Cron\CronExpression')) {
    die("BTK Raporlama Modülü kütüphaneleri yüklendikten sonra bile bulunamadı. Lütfen sunucu yapılandırmanızı kontrol edin.");
}
...
class BtkreportsCsrfHelper
{
    ...
    public static function verify() {
        if (class_exists('\WHMCS\Utility\CSRF')) {
            ...
            if (empty($submittedToken) || empty($_SESSION['btk_legacy_token']) || !hash_equals($_SESSION['btk_legacy_token'], $submittedToken)) {
                die('Invalid Token');
            }
...
function btkreports_activate() {
    try {
        ...
        BtkHelper::logAction('Modül Aktivasyonu', 'BAŞARILI', 'BTK Raporlama Modülü v8.0.9 başarıyla etkinleştirildi.');

        return ['status' => 'success', 'description' => 'BTK Raporlama Modülü başarıyla etkinleştirildi. Lütfen modül ayarlarını yapılandırın.'];
    } catch (Exception $e) {
        $errorMessage = 'Modül etkinleştirilirken kritik bir hata oluştu: ' . $e->getMessage();
        if (function_exists('logActivity')) {
            logActivity("BTK Raporlama Modülü Aktivasyon Hatası: " . $e->getMessage(), 0);
        }
...
function btkreports_output($vars) {
    ...
    } catch (\SmartyException $e) { BtkHelper::logAction('Smarty Hatası (Menu)', 'HATA', $e->getMessage()); }
    ...
    if (isset($templateFile) && file_exists(__DIR__ . '/templates/admin/' . $templateFile)) {
        ...
        catch (\SmartyException $e) { BtkHelper::logAction('Smarty Hatası', 'KRITIK', $e->getMessage() . ' Şablon: ' . $templateFile, $adminId); echo '<div class="alert alert-danger">Şablon yükleme hatası oluştu. Lütfen logları kontrol edin.</div>'; }
    }
    echo '</div>';
}
...
function btk_page_product_group_mappings(&$smarty, $LANG) {
    ...
    $smarty->assign('productGroups', BtkHelper::getWhmcsProductGroups());
    $smarty->assign('activeBtkAuthTypes', BtkHelper::getAktifYetkiTurleri());
...
function btk_page_generate_reports(&$smarty, $LANG) {
    $smarty->assign('page_title', $LANG['generateReportsPageTitle'] ?? 'Manuel Raporlar');
    $smarty->assign('activeBtkAuthTypesForReports', BtkHelper::getAktifYetkiTurleri());
...
function btk_action_save_single_personel($postData, $LANG) {
    $data = $postData['modal_personel_data'] ?? [];
    $data['btk_listesine_eklensin'] = isset($data['btk_listesine_eklensin']) ? 1 : 0;
    return BtkHelper::savePersonel($data, $LANG);
}
...
function btk_action_save_pop_definition($postData, $LANG) {
    $data = $postData['popdata'] ?? [];
    $data['aktif_pasif_durum'] = isset($data['aktif_pasif_durum']) ? 1 : 0;
    $data['izleme_aktif'] = isset($data['izleme_aktif']) ? 1 : 0;
    return BtkHelper::saveIssPopNoktasi($data, $LANG);
}
    ...
    try {
        switch ($ajax_action) {
            ...
            case 'test_ftp_connection':
                $ftpManager = new FtpManager($request_data);
            ...
            case 'validate_tckn_personel':
                ...
                $result = $manager->execute($request_data);
        ...
        BtkHelper::logAction("AJAX Hatası ($ajax_action)", 'HATA', $e->getMessage());
