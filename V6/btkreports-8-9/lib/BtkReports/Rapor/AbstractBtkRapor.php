 ...
 *
 * Sürüm 8.1.0 ile BtkHelper ve yeni temp yolu mantığıyla tam senkronize edilmiştir.
 *
 * @package    WHMCS
...
abstract class AbstractBtkRapor implements BtkRaporInterface
{
    ...
    public function olusturVeGonder(string $ftpType = 'main'): array
    {
        try {
            ...
            // Personel raporu Excel olduğu için sıkıştırılmaz, diğerleri sıkıştırılır.
            if (static::RAPOR_DOSYA_UZANTISI === '.xlsx') {
                $tempFilePath = \BtkReports\Exporter\ExcelExporter::exportPersonelList($data, $filename, false, $this->tempDir);
                $finalFilePath = $tempFilePath; // Sıkıştırma yok
            } else {
                $tempFilePath = $this->createTempFile($filename, $this->formatReportContent($data));
                $finalFilePath = $this->compressFile($tempFilePath, $filename);
                @unlink($tempFilePath); // Ham dosyayı sil
            }

            if (!$finalFilePath) {
                throw new \Exception("Geçici dosya oluşturulamadı veya sıkıştırılamadı.");
            }
            ...
            @unlink($finalFilePath); // Son dosyayı (sıkıştırılmış veya değil) temizle
    ...
    public function olusturVeIndir(bool $outputToBrowser = true): array
    {
        try {
            ...
            // Excel raporları için özel işlem
            if (static::RAPOR_DOSYA_UZANTISI === '.xlsx') {
                return \BtkReports\Exporter\ExcelExporter::exportPersonelList($data, $filename, $outputToBrowser, $this->tempDir) 
                    ? ['status' => true, 'message' => 'Excel raporu oluşturuldu.']
                    : ['status' => false, 'message' => 'Excel dosyası oluşturulurken hata.'];
            }
            
            // Standart .abn raporları
    ...
    protected function createTempFile(string $filename, string $content): string
    {
        if (!is_dir($this->tempDir) && !@mkdir($this->tempDir, 0755, true)) {
            throw new \Exception("Geçici dosya dizini oluşturulamadı: {$this->tempDir}");
        }
