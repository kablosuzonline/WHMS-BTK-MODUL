...
use BtkReports\Helper\BtkHelper; // Loglama için BtkHelper'ı kullan
...
class PopMonitor
{
    ...
    public function checkStatus(): array
    {
        ...
        if (empty($ipToPing) || !filter_var($ipToPing, FILTER_VALIDATE_IP)) {
            BtkHelper::logAction('POP İzleme Hatası', 'UYARI', "POP ID: {$this->pop['id']} için geçerli IP adresi tanımlı değil: {$ipToPing}");
            return ['status' => 'UNKNOWN', 'latency' => null, 'detail' => 'İzlenecek geçerli bir IP adresi tanımlı değil.'];
        }

        if (!function_exists('exec')) {
            BtkHelper::logAction('POP İzleme Hatası', 'KRITIK', "Sunucuda `exec()` fonksiyonu kapalı veya güvenlik kısıtlamaları nedeniyle kullanılamıyor.");
        ...
        // escapeshellarg() ile IP adresini güvenli hale getiriyoruz.
