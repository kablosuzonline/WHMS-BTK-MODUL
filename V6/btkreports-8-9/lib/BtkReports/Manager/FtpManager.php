...
class FtpManager
{
    ...
    private function createRecursiveDir($conn_id, string $path): void
    {
        ...
        foreach ($parts as $part) {
            ...
            // Dizin var mı diye kontrol et, yoksa oluştur
            if (!@ftp_chdir($conn_id, $part)) {
                @ftp_mkdir($conn_id, $part);
                // Oluşturduktan sonra içine girmeyi tekrar dene
                if (!@ftp_chdir($conn_id, $part)) {
                    // Hala girilemiyorsa döngüden çık
                    break;
                }
            }
        }
        // İşlem bittikten sonra en başa dön
        @ftp_chdir($conn_id, '/');
    }
