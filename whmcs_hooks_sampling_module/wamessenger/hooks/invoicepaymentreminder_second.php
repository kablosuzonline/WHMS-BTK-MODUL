...
$hook = array(
    ...
    'function' => 'InvoicePaymentReminder_secondoverdueWa',
    'description' => 'Executes when an automated invoice payment reminder for secondoverdue is sent.<br>Client Related<br>First Name: {firstname}, Last Name: {lastname}, Email Address: {email}, Company Name: {companyname}, Credit Balance: {credit}, Country: {country}, Currency: {currency}<br>Invoice Related<br> Due Date: {duedate}, Total: {total}, Invoice ID: {invoiceid} , Date Created: {date}, Date Paid: {datepaid}, Sub Total: {subtotal}, Tax: {tax}, Tax Rate: {taxrate}, Invoice Status: {status}, Invoice Payment Method: {paymentmethod}, Invoice Payment Link: {invoice_payment_link}, Invoice Pdf Link: {invoice_file}',
    'type' => 'client',
    'extra' => '',
...
if(!function_exists('InvoicePaymentReminder_secondoverdueWa')){
    function InvoicePaymentReminder_secondoverdueWa($args){
