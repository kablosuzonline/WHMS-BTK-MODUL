...
class ExcelExporter {
        ...
        try {
            ...
            if (is_array($personelData)) {
                foreach ($personelData as $personel) {
                    // $personel dizisinin anahtarları $headers ile aynı olmalı.
                    // BtkHelper::getPersonelDataForReport() bu formatta veri hazırlamalı.
    ...
    public static function importIssPopFromExcel($filePath) {
        ...
        try {
            ...
            if ($highestRow < 2) { // En az başlık ve bir veri satırı olmalı (veya sadece başlık ise 1)
            ...
            $normalizedHeaderToColLetter = []; // Excel Sütun Harfi => normalize edilmiş başlık
            foreach ($rawHeaders as $colLetter => $originalHeader) {
                if (!empty(trim((string)$originalHeader))) {
                    // Normalize edilmiş başlığı anahtar, sütun harfini değer yapalım
    ...
    public static function exportGenericList($dataToExport, $filename = 'export.xlsx', $sheetTitle = 'Veri Listesi', $outputToBrowser = true, $savePath = null) {
        if (!$GLOBALS['phpSpreadsheetLoadedCheck']) { /* ... log ... */ return false; }
    ...
    private static function normalizeExcelHeader($header) {
        ...
        // Türkçe karakterleri ve özel karakterleri temizle/dönüştür
        $search = ['ı', 'ğ', 'ü', 'ş', 'ö', 'ç', ' ', '/', '-', '(', ')', '.', "\n", "\r", '[', ']', '̇']; // Noktalı büyük İ'nin küçük hali için de
        ...
        // Bu map, Excel'deki birebir başlığın normalize edilmiş haline (anahtar) ve
        // kod içinde kullanacağımız standart normalize edilmiş forma (değer) eşlenmelidir.
        $map = [
            'pop_adi_lokasyon_adi' => 'pop_adi__lokasyon_adi',
            'pop_ad_lokasyon_ad' => 'pop_adi__lokasyon_adi', // Olası varyasyon
            'yayin_yapilan_ssi_d' => 'yayin_yapilan_ssi_d',
            'ip_adresi_' => 'i_p_adresi_',
            'turu' => 'turu',                                 
            'markasi' => 'markasi',
            'modeli_' => 'modeli_',
            'i_l' => 'i_l',
            'i_lce' => 'i_lce',
            'mahalle_koy' => 'mahalle', // Excel'de "Mahalle/Köy" olabilir
            'mahalle' => 'mahalle',
