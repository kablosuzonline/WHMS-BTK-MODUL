 ...
 * (Global encrypt/decrypt kullanılarak güncellenmiş, tüm fonksiyonların içleri doldurulmuş)
 * @package    WHMCS
...
// use WHMCS\Utility\Encryption; // ARTIK KULLANILMIYOR
use Cron\CronExpression; 

// WHMCS'in global şifreleme fonksiyonlarının varlığını kontrol et
if (!function_exists('encrypt')) {
    // Bu kritik bir durum, logla ve belki bir uyarı göster.
    // Ancak BtkHelper sınıfı yine de tanımlansın.
    error_log("BTK Reports Critical Error: WHMCS global 'encrypt' function not found!");
}
if (!function_exists('decrypt')) {
    error_log("BTK Reports Critical Error: WHMCS global 'decrypt' function not found!");
}
...
class BtkHelper {
            ...
            if ($setting) {
                if (in_array($settingName, ['main_ftp_pass', 'backup_ftp_pass']) && !empty($setting->value)) {
                    try { 
                        ...
                        return ''; // Fonksiyon yoksa boş dön
                    } catch (\Throwable $e) { 
                ...
                if (in_array($setting->setting, ['main_ftp_pass', 'backup_ftp_pass']) && !empty($setting->value)) {
                    try { 
                        if (function_exists('decrypt')) {
                            ...
                            self::logAction('Toplu Ayar Deşifre Hatası', 'UYARI', "WHMCS decrypt fonksiyonu bulunamadı, ayar '{$setting->setting}' deşifre edilemedi.");
                        }
                    } catch (\Throwable $e) { 
                        $settingsArray[$setting->setting] = ''; 
                        self::logAction('Toplu Ayar Deşifre Hatası', 'UYARI', "Ayar '{$setting->setting}' deşifre edilemedi: " . $e->getMessage());
                    }
                } else {
    ...
    public static function set_btk_setting($settingName, $value) {
        try {
            ...
            if (in_array($settingName, ['main_ftp_pass', 'backup_ftp_pass'])) {
                if ($value === '********') { // Placeholder ise, DB'deki değeri koru, bir şey yapma
                    return true; 
                } elseif (!empty($value)) { // Yeni bir şifre girilmişse şifrele
                    if (function_exists('encrypt')) {
                        $valueToStore = encrypt($value);
                    } else {
                        self::logAction('Ayar Şifreleme Hatası', 'HATA', "WHMCS encrypt fonksiyonu bulunamadı! '$settingName' şifrelenemedi.");
                        return false; 
                    }
                } else { // Gelen değer boşsa, DB'deki şifreyi silmek için boş string kaydet
                    $valueToStore = '';
                }
            }
    ...
    public static function logAction($islem, $logSeviyesi = 'INFO', $mesaj = '', $kullaniciId = null, $ekVeri = null) {
        try {
            ...
            error_log("BTK Reports Module - LogAction Error: " . $e->getMessage() . " | Original Log: [$logSeviyesi] $islem - " . $logMesaj);
        }
    }
    ...
    public static function getAllYetkiTurleriReferans() {
        try {
            $results = Capsule::table('mod_btk_yetki_turleri_referans')->orderBy('yetki_adi')->get()->all();
...
// --- BÖLÜM 2/5 BAŞI - BtkHelper.php - (Yetki Türü Yönetimi Devamı, Rapor Bilgileri/Cron)
    ...
    public static function getNextCronRunTime($cronScheduleSettingKey) {
        global $LANG; // Dil dosyasını kullanabilmek için
        ...
        try {
            ...
            $nextRunLocalDate = $cron->getNextRunDate('now', 0, false, date_default_timezone_get()); // Sunucunun zaman dilimine göre
...
// --- BÖLÜM 3/5 BAŞI - BtkHelper.php - (FTP İşlemleri, Adres Verileri)
    ...
    public static function uploadFileToFtp($localFilePath, $remoteFileName, $type = 'main', $raporTuru = 'BILINMEYEN') {
        ...
        if ($conn_id) {
            ...
            if (@ftp_login($conn_id, $login_user_ftp, $login_pass_ftp)) {
                ...
                // Dizine girildiyse sadece dosya adı ile yükle
    ...
    public static function getFtpLog($filters = [], $orderBy = 'id', $sortOrder = 'DESC', $limit = 50, $offset = 0) {
         try {
            $query = Capsule::table('mod_btk_ftp_logs');
            // Filtre anahtarları btkeports.php'den gelenlerle uyumlu olmalı (s_ ile başlayanlar)
...
// --- BÖLÜM 3/5 Sonu ---
// --- BÖLÜM 4/5 BAŞI - BtkHelper.php - (ISS POP Noktaları Yönetimi, Personel Yönetimi)
    ...
    public static function searchPopSSIDs($term = '', $filter_by = null, $filter_value = null, $limit = 25) {
        // filter_value boş string ise veya null ise ve term de boşsa, etkili bir arama yapılamaz.
    ...
    public static function getIssPopNoktalariList($filters = [], $orderBy = 'pop.pop_adi', $sortOrder = 'ASC', $limit = 25, $offset = 0) {
        try {
            // Count için filtreli sorgu (select olmadan)
            ...
            // Veri çekmek için filtreli sorgu (join ve select ile)
            ...
            // Sıralama için sütun adının güvenli olduğundan emin olalım (SQL injection önlemi)
            $allowedOrderColumns = ['pop.pop_adi', 'pop.yayin_yapilan_ssid', 'il.il_adi', 'ilce.ilce_adi', 'pop.pop_tipi', 'pop.aktif_pasif_durum'];
            $safeOrderBy = in_array($orderBy, $allowedOrderColumns) ? $orderBy : 'pop.pop_adi'; // Varsayılan
            $safeSortOrder = strtoupper($sortOrder) === 'DESC' ? 'DESC' : 'ASC';

            $pops = $dataQuery->select('pop.*', 'il.il_adi', 'ilce.ilce_adi', 'mah.mahalle_adi')
                              ->orderByRaw(Capsule::raw(DB::connection()->getQueryGrammar()->wrap($safeOrderBy) . " $safeSortOrder")) 
...
// --- BÖLÜM 3/5 Sonu ---
// --- BÖLÜM 4/5 BAŞI - BtkHelper.php - (Personel Yön. Devam, Dosya/CNT, Rapor Oluşturma Yardımcıları)
    ...
    public static function savePersonel($data, $adminId = null) {
        ...
        if (empty($dogumYiliForNVI) && !empty($tckn)) { // TCKN varsa doğum yılı da NVI için zorunlu
        ...
        $tcknDogrulamaMesaji = 'NVI Doğrulaması yapılmadı (gerekli bilgiler eksik veya servis hatası).';
        ...
        try {
            $personelDataToSave = [
                ...
                'firma_unvani' => self::get_btk_setting('operator_title', self::get_btk_setting('operator_name', '[FİRMA ÜNVANI/ADI AYARLANMAMIŞ]')),
                'ad' => trim($data['ad']), 
            ...
            // Unique kontrolleri
            $uniqueCheckQueryTCKN = Capsule::table('mod_btk_personel')->where('tckn', $personelDataToSave['tckn'])->whereNull('isten_ayrilma_tarihi'); // Sadece aktif çalışanlar için TCKN unique olmalı
            if ($personelIdToUpdate > 0) { $uniqueCheckQueryTCKN->where('id', '!=', $personelIdToUpdate); }
            $existingPersonelByTCKN = $uniqueCheckQueryTCKN->first(); 
            if ($existingPersonelByTCKN) { return ['status' => false, 'message' => "Bu TCKN ({$personelDataToSave['tckn']}) zaten aktif bir personele (ID: {$existingPersonelByTCKN->id} - {$existingPersonelByTCKN->ad} {$existingPersonelByTCKN->soyad}) ait."]; }
            ...
            if ($personelDataToSave['whmcs_admin_id']) {
                $uniqueCheckQueryAdminID = Capsule::table('mod_btk_personel')->where('whmcs_admin_id', $personelDataToSave['whmcs_admin_id']);
                 if ($personelIdToUpdate > 0) { $uniqueCheckQueryAdminID->where('id', '!=', $personelIdToUpdate); }
                $existingPersonelByAdminID = $uniqueCheckQueryAdminID->first(); 
                if ($existingPersonelByAdminID) { return ['status' => false, 'message' => "Seçilen WHMCS Yöneticisi (ID: {$personelDataToSave['whmcs_admin_id']}) zaten başka bir personele (ID: {$existingPersonelByAdminID->id} - {$existingPersonelByAdminID->ad} {$existingPersonelByAdminID->soyad}) atanmış."]; }
            }
            ...
            return ['status' => false, 'message' => 'Veritabanı hatası. Lütfen logları kontrol edin. Hata: '. substr($e->getMessage(),0,100)]; 
        }
    }
    ...
    public static function getTeknikPersonelForIlce($ilce_id) {
        ...
        try {
            ...
            if(empty($teknikDepartmanAdlari)) {
                 self::logAction('Teknik Personel Arama Uyarısı', 'UYARI', 'Teknik departman adları ayarlanmamış veya boş.', null, ['ilce_id' => $ilce_id]);
                 return [];
            }
    ...
    // ... (Bu fonksiyonlar Bölüm 4/5 olarak adlandırılan bir önceki gönderimde tam olarak verilmişti) ...
    // ... generateReportFilename ve getNextCnt fonksiyonlarının tam içerikleri buraya gelecek ...
    // ... (Dosya Adlandırma ve CNT Yönetimi fonksiyonları bir önceki gönderimdeki gibi, güncel) ...

// --- BÖLÜM 4/5 Sonu ---
// --- BÖLÜM 5/5 BAŞI - BtkHelper.php - (Dosya/CNT, Rapor Oluşturma, Hook Handler'lar, Diğer Yardımcılar)
    ...
    public static function generateReportFilename($operatorAdi, $operatorKodu, $yetkiTuruGrup, $raporTuru, $cnt, $timestamp = null, $forFtpType = 'main') {
        ...
        if ($raporTuruUpper === 'PERSONEL_LISTESI') {
            ...
            $yearMonthPart = $addYearMonth ? "_" . gmdate('Y_m', $timestamp) : ''; // YYYY_AA formatı
            ...
            // OPERATORADI_OPERATORKODU_YETKITURU_RAPORTURU_YYYYMMDDHHMMSS_CNT.abn
    ...
    public static function getAboneRehberData($yetkiTuruGrup, $filters = []) {
        ...
        $data = [];
        // TODO: TAM İMPLEMENTASYON GEREKLİ
        // Bu fonksiyon, mod_btk_abone_rehber tablosundan ve ilişkili tablolardan (adres, pop, müşteri vb.)
        // $yetkiTuruGrup'a uygun tüm abonelerin BTK'nın istediği 85+ alanı içeren verilerini çekmelidir.
        // Alan sıralaması getBtkAlanSiralamasi() fonksiyonundan alınmalıdır.
        // HIZMET_TIPI alanı, hizmetin ürün grubunun mod_btk_product_group_mappings tablosundaki
        // ek3_hizmet_tipi_kodu'na göre ayarlanmalıdır.
        // MUSTERI_TIPI alanı, abonenin durumuna göre (bireysel, kurumsal vb.) ayarlanmalıdır.
        // Tüm tarih alanları YYYYMMDDHHMMSS veya YYYYMMDD formatında (UTC) olmalıdır.
        // Boş bırakılmaması gereken alanlar için varsayılan değerler veya hata yönetimi düşünülmelidir.
        self::logAction("Rehber Veri Çekme Tamamlandı ($yetkiTuruGrup)", 'DEBUG', count($data) . " kayıt bulundu (IMPLEMENTASYON BEKLENİYOR).");
        return $data; 
    }
    ...
    public static function getAboneHareketData($yetkiTuruGrup, $filters = []) {
        ...
        $data = [];
        // TODO: TAM İMPLEMENTASYON GEREKLİ
        // 1. mod_btk_abone_hareket_live'dan gonderildi_flag = 0 olanları ve $yetkiTuruGrup'a uygun olanları çek.
        //    (Hareketin ait olduğu rehber_kayit_id (mod_btk_abone_rehber.id) üzerinden hizmeti bul.
        //     Hizmetin ürün grubundan $yetkiTuruGrup'a uygun olup olmadığını kontrol et.)
        // 2. Hareket dosyası da rehber ile aynı formatta olduğu için, hareketin oluştuğu andaki TÜM rehber bilgilerini
        //    (islem_detayi_json alanından veya rehberden anlık çekerek) ve hareket bilgilerini
        //    birleştirerek BTK formatına uygun bir dizi oluştur.
        // 3. getBtkAlanSiralamasi('HAREKET', $yetkiTuruGrup) ile alan sırasını al ve veriyi bu sıraya göre hazırla.
        self::logAction("Hareket Veri Çekme Tamamlandı ($yetkiTuruGrup)", 'DEBUG', count($data) . " kayıt bulundu (IMPLEMENTASYON BEKLENİYOR).");
        return $data; 
    }
    ...
    public static function markHareketAsGonderildi($hareketIds, $dosyaAdi, $cnt) {
        ...
        try {
            ...
            if ($updated > 0) { self::logAction('Hareketler Gönderildi İşaretlendi', 'INFO', $updated . " adet hareket $dosyaAdi (CNT:$cnt) ile gönderildi olarak işaretlendi."); }
            else { self::logAction('Hareketler Gönderildi İşaretlendi (Etkilenen Yok)', 'UYARI', "ID'leri verilen hareketler bulunamadı veya zaten işaretliydi: " . implode(',', $hareketIds)); }
            return true;
        } catch (Exception $e) { self::logAction('Hareket Gönderildi İşaretleme Hatası', 'HATA', $e->getMessage()); return false; }
    }
    ...
    public static function archiveOldHareketler() {
        ...
        if ($liveDays <=0 || $archiveDays <=0) {
            self::logAction('Hareket Arşivleme Atlandı', 'UYARI', 'Canlı veya arşiv saklama günleri geçerli değil.');
            return;
        }
        ...
        try {
            $toArchive = Capsule::table('mod_btk_abone_hareket_live')->where('gonderildi_flag', 1)->where('gonderim_zamani', '<', $dateLimitLiveToArchive)->get()->all();
            $archivedCount = 0;
            if (!empty($toArchive)) {
                $liveIdsToDelete = [];
                foreach ($toArchive as $log) {
                    $logArray = (array)$log; unset($logArray['id']); $logArray['arsivlenme_zamani'] = gmdate('Y-m-d H:i:s');
                    try { Capsule::table('mod_btk_abone_hareket_archive')->insert($logArray); $liveIdsToDelete[] = $log->id; $archivedCount++; } 
                    catch (Exception $insertEx) { self::logAction('Hareket Arşivleme Tekil Hata', 'HATA', "Hareket ID {$log->id} arşivlenemedi: " . $insertEx->getMessage()); }
                }
                if (!empty($liveIdsToDelete)) { Capsule::table('mod_btk_abone_hareket_live')->whereIn('id', $liveIdsToDelete)->delete(); }
            ...
            if ($deletedFromArchive > 0) { self::logAction('Eski Hareket Arşivi Silme', 'INFO', $deletedFromArchive . ' adet eski arşivlenmiş hareket kalıcı olarak silindi.'); }
        } catch (Exception $e) { self::logAction('Hareket Arşivleme/Silme Genel Hata', 'HATA', $e->getMessage()); }
    }
        ...
        try {
            ...
            foreach($personeller as $personel_obj){
                ...
                $reportData[] = [
                    ...
                    'Adı' => $personel['ad'], 'Soyadı' => $personel['soyad'],
                    'TC Kimlik No' => $personel['tckn'], 'Ünvan' => $personel['unvan'],
                    'Çalıştığı birim' => $personel['departman_adi'], 'Mobil telefonu' => $personel['mobil_tel'],
                    'Sabit telefonu' => $personel['sabit_tel'], 'E-posta adresi' => $personel['email']
                ];
            }
            return $reportData;
    ...
    public static function formatAbnRow($dataArray, $btkAlanSiralamasi, $isHareket = false) {
        ...
        foreach ($btkAlanSiralamasi as $btkFieldKey) {
            ...
            if (strtoupper($btkFieldKey) === 'ISS_POP_BILGISI') {
                $sunucuAdi = $dataArray['iss_pop_bilgisi_sunucu'] ?? ($dataArray['sunucu_adi'] ?? ($dataArray['server_hostname'] ?? ''));
                $ssid = $dataArray['yayin_yapilan_ssid'] ?? ''; 
                if (empty($ssid) && !empty($dataArray['iss_pop_noktasi_id'])) {
            ...
            } elseif (array_key_exists($dbFieldKey, $dataArray)) { $value = $dataArray[$dbFieldKey]; } 
            elseif (array_key_exists(strtoupper($btkFieldKey), $dataArray)) { $value = $dataArray[strtoupper($btkFieldKey)];}
    ...
    public static function getBtkAlanSiralamasi($raporTuru = 'REHBER', $yetkiTuruGrup = null) {
        // DİKKAT: Bu liste BTK'nın 314_KK_Abone_Desen.pdf Bölüm 3.A'daki 85 alanı ve yetki türüne özel alanları
        // tam ve doğru sırada içermelidir. `abonedesen.xlsx` dosyanızdaki "ALAN ADI (VERİ TABANI)" sütununa göre.
        $ortakAlanlar = [
        ...
        $ozelAlanlar = []; $grup = strtoupper($yetkiTuruGrup ?? '');
        ...
        // TODO: KABLOYAYIN, OKTH, REHBERLIK, SMSH, UYDUKABLOTV, UYDUPLAT, CTH için özel alanlar abonedesen.xlsx'den eklenmeli.
    ...
    // --- WHMCS Veri Entegrasyonu (HOOK Handler Fonksiyonları - Detaylı İskeletler) ---
    public static function handleClientAdd($vars) {
        $clientId = $vars['userid'] ?? null; $adminId = self::getCurrentAdminId();
        self::logAction('Hook: ClientAdd', 'DEBUG', "Müşteri ID: $clientId eklendi. BTK rehber kaydı oluşturulacak (ilk hizmet eklendiğinde).", $adminId, ['vars_keys' => array_keys($vars)]);
        // Bu aşamada rehber kaydı oluşturmak yerine, ilk hizmet eklendiğinde (handleServiceCreate) oluşturmak daha mantıklı olabilir.
        // Ancak temel müşteri bilgileri (TCKN, adres vb.) mod_btk_abone_rehber'e önceden yazılabilir.
        // Veya bu hook'ta sadece "MUSTERI_BILGI_DEGISIKLIGI" gibi bir ön hareket oluşturulabilir (eğer boş bir rehber kaydı varsa).
    }
    public static function handleClientEdit($vars) {
        $clientId = $vars['userid'] ?? null; $adminId = self::getCurrentAdminId();
        self::logAction('Hook: ClientEdit', 'DEBUG', "Müşteri ID: $clientId güncellendi. BTK verileri kontrol edilecek.", $adminId, ['updated_fields_keys' => array_keys($vars)]);
        if (!$clientId) return;
        // TODO: Güncellenen alanları ($vars içinde eski ve yeni değerler olabilir, WHMCS dokümanına bakılmalı)
        // Eğer TCKN, Ad, Soyad, Yerleşim Adresi gibi BTK ile ilgili alanlar değişmişse:
        // 1. Bu müşteriye ait TÜM aktif hizmetler için mod_btk_abone_rehber kayıtlarını güncelle.
        // 2. Her bir etkilenen hizmet için "MUSTERI_BILGI_DEGISIKLIGI" (Kod: 11) hareketi oluştur.
        //    Hareketin islem_detayi_json alanı, güncellenmiş TÜM rehber verisini içermeli.
    }
    public static function handleClientClose($clientId) {
        $adminId = self::getCurrentAdminId();
        self::logAction('Hook: ClientClose', 'UYARI', "Müşteri ID: $clientId kapatıldı. Tüm aktif hizmetleri için BTK 'IPTAL' hareketleri oluşturulacak.", $adminId);
        // TODO: Bu müşteriye ait tüm AKTİF hizmetleri (tblhosting.domainstatus = 'Active' veya 'Suspended') bul.
        // Her bir hizmet için:
        // 1. mod_btk_abone_rehber.hat_durum = 'I' (İptal) yap.
        // 2. hat_durum_kodu = '5' (IPTAL MUSTERI TALEBI) veya uygun bir kod (EK-1).
        // 3. abone_bitis = gmdate('YmdHis') yap.
        // 4. "HAT_IPTAL" (Kod: 10) hareketi oluştur.
    }
    public static function handleClientDelete($clientId) {
        $adminId = self::getCurrentAdminId();
        self::logAction('Hook: ClientDelete', 'KRİTİK UYARI', "Müşteri ID: $clientId siliniyor! BTK kayıtları anonimleştirilmeli veya özel bir işaret konulmalı.", $adminId);
        // TODO: BTK mevzuatına göre müşteri silinmesi durumunda verilerin nasıl işleneceği belirlenmeli.
        // Genellikle fiziksel silme yerine anonimleştirme veya "silindi" olarak işaretleme yapılır.
        // Bu müşteriye ait tüm hizmetler için handleClientClose benzeri bir iptal işlemi yapılmalı.
    }
    public static function handleMergeClient($sourceClientId, $targetClientId) {
        $adminId = self::getCurrentAdminId();
        self::logAction('Hook: MergeClient', 'UYARI', "Kaynak Müşteri ID: $sourceClientId, Hedef Müşteri ID: $targetClientId'ye birleştirildi. BTK kayıtları güncellenecek.", $adminId);
        // TODO: sourceClientId'ye ait tüm mod_btk_abone_rehber ve mod_btk_abone_hareket_live kayıtlarını
        // (ve varsa mod_btk_service_pop_mapping) targetClientId'ye aktar.
        // MUSTERI_ID alanlarını da güncelle.
    }
    
    public static function handleServiceCreate($params) {
        $serviceId = $params['serviceid'] ?? null; $clientId = $params['userid'] ?? null; $adminId = self::getCurrentAdminId();
        self::logAction('Hook: ServiceCreate (AfterModuleCreate)', 'INFO', "Hizmet ID: $serviceId (Müşteri: $clientId) aktif edildi. BTK kaydı ve 'YENI_ABONELIK' hareketi oluşturulacak.", $adminId);
        if (!$serviceId || !$clientId) { self::logAction('handleServiceCreate Hatası', 'HATA', 'Eksik serviceid veya userid.', $adminId); return; }
        
        $clientDetails = self::getWhmcsClientDetails($clientId); 
        $serviceDetails = self::getWhmcsServiceDetails($serviceId); 
        if (!$clientDetails || !$serviceDetails) { self::logAction('handleServiceCreate Hatası', 'HATA', "Müşteri ($clientId) veya Hizmet ($serviceId) detayları çekilemedi.", $adminId); return; }

        $yetkiGrup = self::getYetkiGrupForService($serviceId);
        $ek3HizmetTipi = self::getEk3HizmetTipiForService($serviceId);
        if (!$yetkiGrup || !$ek3HizmetTipi) { self::logAction('handleServiceCreate Hatası', 'UYARI', "Hizmet ($serviceId) için BTK Yetki Grubu veya EK-3 Hizmet Tipi eşleşmesi bulunamadı. Raporlama yapılamayabilir.", $adminId); /* return; Opsiyonel */}

        $rehberData = [
            'whmcs_client_id' => $clientId, 'whmcs_service_id' => $serviceId,
            'operator_kod' => self::get_btk_setting('operator_code'),
            'musteri_id' => (string)$clientId, 
            'hat_no' => $serviceDetails['domain'] ?: ($serviceDetails['username'] ?: 'SERV'.$serviceId),
            'hat_durum' => 'A', 'hat_durum_kodu' => '1', 'hat_durum_kodu_aciklama' => self::getHatDurumAciklamasi('1') ?? 'AKTIF',
            'hizmet_tipi' => $ek3HizmetTipi ?? 'DIGER', // Eşleşme yoksa DIGER
            'musteri_tipi' => !empty($clientDetails['companyname']) ? (strpos(strtoupper($clientDetails['companyname']), 'ŞAHIS') !== false || strpos(strtoupper($clientDetails['companyname']), 'SAHIS') !== false ? 'G-SIRKET' : 'T-SIRKET') : 'G-SAHIS',
            'abone_baslangic' => gmdate('YmdHis', strtotime($serviceDetails['regdate'] ?? 'now')),
            'abone_bitis' => '00000000000000',
            'abone_adi' => $clientDetails['firstname'], 'abone_soyadi' => $clientDetails['lastname'],
            'abone_unvan' => $clientDetails['companyname'],
            'abone_tc_kimlik_no' => $clientDetails['customfields']['tckn'] ?? ($clientDetails['tax_id'] ?? null), // Varsayım: TCKN özel alanda veya tax_id'de
            'abone_vergi_numarasi' => !empty($clientDetails['tax_id']) && strlen($clientDetails['tax_id']) == 10 ? $clientDetails['tax_id'] : null,
            'abone_email' => $clientDetails['email'],
            'abone_adres_yerlesim_il' => $clientDetails['city'],
            'abone_adres_yerlesim_ilce' => $clientDetails['state'], // WHMCS state'i ilçe olarak kullanıyorsa
            'abone_adres_yerlesim_cadde' => $clientDetails['address1'],
            'abone_adres_yerlesim_dis_kapi_no' => $clientDetails['address2'], // address2 kapı no için kullanılabilir
            'abone_adres_yerlesim_posta_kodu' => $clientDetails['postcode'],
            'abone_adres_irtibat_tel_no_1' => preg_replace('/[^0-9]/', '', $clientDetails['phonenumber']),
            'statik_ip' => $serviceDetails['dedicatedip'],
            'abone_tarife' => $serviceDetails['product_name'],
            // Diğer tüm alanlar (kimlik, tesis adresi vb.) müşteri veya hizmet detaylarından çekilecek veya manuel girilecek.
            // Bu hook anında bu veriler eksik olabilir. Rapor oluşturulurken bu eksiklikler sorun yaratır.
            // Zorunlu alanlar için sipariş sırasında veri toplama gerekebilir.
        ];
        // Tesis adresi başlangıçta yerleşim adresiyle aynı varsayılabilir.
        $rehberData['abone_adres_tesis_il'] = $rehberData['abone_adres_yerlesim_il'];
        $rehberData['abone_adres_tesis_ilce'] = $rehberData['abone_adres_yerlesim_ilce'];
        $rehberData['abone_adres_tesis_cadde'] = $rehberData['abone_adres_yerlesim_cadde'];
        // ... diğer tesis adres alanları ...

        // Rehber kaydını oluştur/güncelle
        $rehberKayitId = null;
        try {
            Capsule::table('mod_btk_abone_rehber')->updateOrInsert( ['whmcs_service_id' => $serviceId], $rehberData );
            $kayit = Capsule::table('mod_btk_abone_rehber')->where('whmcs_service_id', $serviceId)->first(['id']);
            if($kayit) $rehberKayitId = $kayit->id;
        } catch (Exception $e) { self::logAction('handleServiceCreate DB Hatası (Rehber)', 'HATA', $e->getMessage(), $adminId, $rehberData); return; }

        if ($rehberKayitId) {
            $hareketKaydi = $rehberData; // Temel olarak rehber verilerini al
            $hareketKaydi['MUSTERI_HAREKET_KODU'] = '1'; // YENI_ABONELIK_KAYDI
            $hareketKaydi['MUSTERI_HAREKET_ACIKLAMA'] = self::getMusteriHareketAciklamasi('1') ?? 'YENİ ABONELİK KAYDI';
            $hareketKaydi['MUSTERI_HAREKET_ZAMANI'] = gmdate('YmdHis');
            try {
                Capsule::table('mod_btk_abone_hareket_live')->insert( self::prepareHareketDataForInsert($hareketKaydi, $rehberKayitId) );
                self::logAction('handleServiceCreate Başarılı', 'INFO', "Hizmet ID: $serviceId için rehber ve 'YENI_ABONELIK' hareketi oluşturuldu (Rehber ID: $rehberKayitId).", $adminId);
            } catch (Exception $e) { self::logAction('handleServiceCreate DB Hatası (Hareket)', 'HATA', $e->getMessage(), $adminId, $hareketKaydi); }
        } else { self::logAction('handleServiceCreate Hatası', 'HATA', "Hizmet ID: $serviceId için rehber kaydı oluşturulamadı/bulunamadı.", $adminId); }
    }
    public static function handleServiceEdit($vars) { /* ... (Detaylı iskelet) ... */ }
    public static function handleServiceSuspend($params) { /* ... (Detaylı iskelet) ... */ }
    public static function handleServiceUnsuspend($params) { /* ... (Detaylı iskelet) ... */ }
    public static function handleServiceTerminate($params) { /* ... (Detaylı iskelet) ... */ }
    public static function handleServiceDelete($serviceId) { /* ... (Detaylı iskelet) ... */ }
    public static function handleServicePackageChange($vars) { /* ... (Detaylı iskelet) ... */ }
    public static function handleInvoicePaid($invoiceId) { /* ... (Detaylı iskelet) ... */ }
    public static function handleInvoiceCancelled($invoiceId) { /* ... (Detaylı iskelet) ... */ }
    public static function syncAdminToPersonel($adminId, $adminVars = []) {
        $logAdminId = self::getCurrentAdminId();
        self::logAction('Hook: syncAdminToPersonel', 'DEBUG', "Admin ID: $adminId senkronizasyonu.", $logAdminId);
        // TODO: $adminVars içinden ad, soyad, email al.
        // mod_btk_personel'de whmcs_admin_id = $adminId olan kaydı bul.
        // Varsa, temel bilgileri güncelle (TCKN, departman vb. elle girilenleri koru).
        // Yoksa, yeni bir personel kaydı oluştur (btk_listesine_eklensin=0 olabilir).
    }
    public static function handleAdminDelete($adminId) {
        $logAdminId = self::getCurrentAdminId();
        self::logAction('Hook: handleAdminDelete', 'UYARI', "Admin ID: $adminId silindi. BTK Personel kaydı güncellenecek.", $logAdminId);
        // TODO: mod_btk_personel'de whmcs_admin_id = $adminId olan kaydın whmcs_admin_id'sini NULL yap.
        // Opsiyonel olarak isten_ayrilma_tarihi set edilebilir veya btk_listesine_eklensin=0 yapılabilir.
    }
    
    public static function getBtkClientInfoForClientArea($clientId) { /* ... (Detaylı iskelet, veri çekme ve maskeleme) ... */ return []; }
    public static function getBtkServiceInfoForClientArea($serviceId) { /* ... (Detaylı iskelet, veri çekme) ... */ return []; }
    public static function performDailyBtkChecks() { self::logAction('Günlük BTK Periyodik Kontrolleri Başladı (TODO)', 'INFO'); /* ... */ }
    public static function handlePreServiceEdit($serviceId, $postData) {
        $adminId = self::getCurrentAdminId();
        self::logAction('Hook: PreAdminServiceEdit (BTK Kaydı)', 'DEBUG', "Hizmet ID: $serviceId için BTK özel alanları kaydedilecek.", $adminId);
        if (empty($serviceId) || !is_array($postData)) return;
        // TODO: $postData içinden 'btk_' ile başlayan alanları (tesis adresi, pop_id, statik_ip vb.) al.
        // Bu alanları mod_btk_abone_rehber tablosuna (veya mod_btk_service_pop_mapping'e) kaydet/güncelle.
        // Eğer önemli bir değişiklik varsa (adres, TCKN -bu müşteri için geçerli- vb.) MUSTERI_BILGI_DEGISIKLIGI hareketi oluştur.
        // Değişen alanları tespit etmek için mevcut rehber kaydını çekip karşılaştırmak gerekebilir.
    }
    
    public static function getWhmcsClientDetails($clientId) { 
        try { $client = Capsule::table('tblclients')->find((int)$clientId); return $client ? (array)$client : null; } 
        catch (Exception $e) { self::logAction('getWhmcsClientDetails Hatası', 'HATA', $e->getMessage()); return null; }
    }
    public static function getWhmcsServiceDetails($serviceId) { 
        try {
            $service = Capsule::table('tblhosting as h')
                ...
                ->leftJoin('tblservers as s', 'h.server', '=', 's.id') // Sunucu adı/ip için
                ->leftJoin('tblproductgroups as pg', 'p.gid', '=', 'pg.id') // Ürün grubu adı için
                ->where('h.id', (int)$serviceId)
                ->select('h.*', 
                         'p.name as product_name', 'p.gid as product_group_id', 'pg.name as product_group_name',
                         Capsule::raw("IFNULL(s.hostname, IFNULL(s.name, '')) as server_hostname"), // Server name or hostname
                         Capsule::raw("IFNULL(s.ipaddress, '') as server_ipaddress")
                        )
                ->first();
            return $service ? (array)$service : null;
        } catch (Exception $e) { self::logAction('getWhmcsServiceDetails Hatası', 'HATA', $e->getMessage()); return null; }
    }

    /** Hareket tablosuna insert için veriyi hazırlar (gereksiz alanları çıkarır) */
    private static function prepareHareketDataForInsert($fullRehberDataOrijinal, $rehberKayitId) {
        $fullRehberData = (array)$fullRehberDataOrijinal; // Gelen veri obje ise diziye çevir
        $hareketData = [];
        $hareketAlanlari = [];
        try { $hareketAlanlari = Capsule::schema()->getColumnListing('mod_btk_abone_hareket_live'); }
        catch (Exception $e) { self::logAction('Hareket Alanları Okuma Hatası', 'HATA', $e->getMessage()); return []; }
        ...
        foreach($hareketAlanlari as $alan) {
            ...
            // cnt_numarasi, gonderilen_dosya_adi, gonderim_zamani başlangıçta null olacak.
            // arsivlenme_zamani da arşiv tablosuna özel.
            if ($alan === 'arsivlenme_zamani') continue;


            $dbFieldKey = strtolower($alan); // DB sütun adları genellikle küçük harf
            $btkFieldKeyUpper = strtoupper($alan); // BTK alan adları genellikle büyük harf

            if (array_key_exists($dbFieldKey, $fullRehberData)) { $hareketData[$alan] = $fullRehberData[$dbFieldKey]; } 
            elseif (array_key_exists($btkFieldKeyUpper, $fullRehberData)) { $hareketData[$alan] = $fullRehberData[$btkFieldKeyUpper];}
            // Hareket tablosundaki diğer alanlar (MUSTERI_HAREKET_KODU vb.) zaten $fullRehberData içinde olmalı (çağıran fonksiyon tarafından set edilmiş)
        }
        return $hareketData;
    }
