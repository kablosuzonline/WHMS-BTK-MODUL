 ...
 * @return array
 */
function intserver_ConfigOptions()
{
    return array( 
 ...
 */
function intserver_CreateAccount(array $params)
{
	...
	$userGroups = intserver_groupgetir($api, $params);
...
//	logModuleCall( "intserver", 'TEST', $postfields, $sonuc[result][0][id] );
	
	

	
	
	$result = intserver_bupdate($params);
...
function intserver_bupdate($params) {
//	intserver_AdminServicesTabFieldsSave($params);
//	<input type="submit" value="Git" class="btn btn-success btn-sm" />
		require_once 'src/KerioControlApi.php';	
	$api = new KerioControlApi('Create user', 'KablosuzOnline', '1.0');	 	
//	intserver_req( $params, $postfields);
...
function intserver_groupgetir($api, $params) {
...
//	logModuleCall( "intserver", 'TEST', $groupgetir, $groupgetir );
	$i = 0;
	...
	foreach ($groupgetir['list'] as $group) {
		...
		$i++;
	//	logModuleCall( "intserver", 'TEST', $groupid, $groupid );
			}	
		}
		if (isset($session)) $api->logout();
	return $userGroups;
 ...
 * @return string "success" or an error message
 */
function intserver_SuspendAccount(array $params)
{
    try {
      	require_once 'src/KerioControlApi.php';	
		$api = new KerioControlApi('Create user', 'KablosuzOnline', '1.0');	
 ...
 * @return string "success" or an error message
 */
function intserver_UnsuspendAccount(array $params)
{
    try {
		$postfields = array();
...
function intserver_ChangeMac($params) {
 ...
 * @return string "success" or an error message
 */
function intserver_TerminateAccount(array $params)
{
    try {
    	require_once 'src/KerioControlApi.php';	
		$api = new KerioControlApi('Create user', 'KablosuzOnline', '1.0');	
 ...
 * @return string "success" or an error message
 */
function intserver_ChangePassword(array $params)
{
    try {
			$username = $params["username"];
			$password = $params["password"];
 ...
 * @return string "success" or an error message
 */
function intserver_ChangePackage(array $params)
{
    try {
 $postfields = array();
	$postfields["program"] = "modify-domain";
...
//	$result = intserver_req( $params, $postfields );
	...
	$users = $api->sendRequest('Users.get', $paramskerio);
	$groupids = intserver_groupgetir($api, 'FILITRESIZ');
	$date = date('d/m/Y', time());
...
function intserver_fupdate($params) {
//	intserver_AdminServicesTabFieldsSave($params);
//	<input type="submit" value="Git" class="btn btn-success btn-sm" />
		require_once 'src/KerioControlApi.php';	
	$api = new KerioControlApi('Create user', 'KablosuzOnline', '1.0');	 	
...
//	update_query( 'tblhosting', $params, array( 'id' => $id ));
	...
	$userGroups = intserver_groupgetir($api, $params); //array('id' => '');
...
function intserver_UsageUpdate($params) {
	

	$postfields = array();
	$postfields["program"] = "list-domains";
	$postfields["multiline"] = "";
	//$result = intserver_req( $params, $postfields, true );
	$dataarray = explode( "
", $result );
...
function intserver_TestConnection(array $params)
{
    try {
 ...
 *
 * @see intserver_buttonOneFunction()
 *
 * @return array
 */
function intserver_AdminCustomButtonArray() {
 ...
 * @return array
 */
function intserver_ClientAreaCustomButtonArray()
{
    return array(
 ...
 *
 * @see http://docs.whmcs.com/Provisioning_Module_SDK_Parameters
 * @see intserver_AdminCustomButtonArray()
 *
 * @return string "success" or an error message
 */
function intserver_buttonOneFunction(array $params)
{
    try {
 ...
 *
 * @see http://docs.whmcs.com/Provisioning_Module_SDK_Parameters
 * @see intserver_ClientAreaCustomButtonArray()
 *
 * @return string "success" or an error message
 */
function intserver_actionOneFunction(array $params)
{
    try {
        // Call the service's function, using the values provided by WHMCS in
        // `$params`.
 ...
 *
 * @see http://docs.whmcs.com/Provisioning_Module_SDK_Parameters
 * @see intserver_AdminServicesTabFieldsSave()
 *
 * @return array
 */
function intserver_AdminServicesTabFields(array $params)
{
    try {
        // Call the service's function, using the values provided by WHMCS in
        // `$params`.
        ...
        return array(
            ...
            'Something Editable' => '<input type="hidden" name="intserver_original_uniquefieldname" '
                . 'value="' . htmlspecialchars($response['textvalue']) . '" />'
                . '<input type="text" name="intserver_uniquefieldname"'
                . 'value="' . htmlspecialchars($response['textvalue']) . '" />',
        );
    } catch (Exception $e) {
        // Record the error in WHMCS's module log.
 ...
 *
 * @see http://docs.whmcs.com/Provisioning_Module_SDK_Parameters
 * @see intserver_AdminServicesTabFields()
 */
function intserver_AdminServicesTabFieldsSave(array $params)
{
    // Fetch form submission variables.
    $originalFieldValue = isset($_REQUEST['intserver_original_uniquefieldname'])
        ? $_REQUEST['intserver_original_uniquefieldname']
        : '';

    $newFieldValue = isset($_REQUEST['intserver_uniquefieldname'])
        ? $_REQUEST['intserver_uniquefieldname']
        : '';
 ...
 * @return array
 */
function intserver_ServiceSingleSignOn(array $params)
{
    try {
 ...
 * @return array
 */
function intserver_AdminSingleSignOn(array $params)
{
    try {
 ...
 * @return array
 */
function intserver_ClientArea(array $params)
{
