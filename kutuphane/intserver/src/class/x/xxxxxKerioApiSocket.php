...
include_once(dirname(__FILE__) . '/KerioApiSocketInterface.php');
...
class KerioApiSocket implements KerioApiSocketInterface {
	...
	//	$this->socketHandler = @fsockopen($this->hostname, $this->port, $errno, $errstr, $this->timeout);
	 ...
	 */
	public function send($data, $param2, $param3) {
				
		if ($this->checkConnection($param2, $param3)) {
			$this->open();
			...
			throw new KerioApiException(sprintf("Cannot connect to %s using port %d.",$param2, $param3));
		}
	}
	 ...
	 * @return	boolean	True on success
	 */
	public final function checkConnection($param2, $param3) {
		if ($this->checkHost()) {
			$socket = @fsockopen($param2, $param3, $errno, $errstr, $this->timeout);
			$this->errorCode = $errno;
			$this->errorMessage = $errstr;
			// $socket;//($socket) ? TRUE : FALSE;
		if($socket){return true;}else{return FALSE;}
		}
		else {
			return FALSE;
		}
	...
	return true;//gethostbyname($hostname);// ? TRUE : FALSE;
	}
