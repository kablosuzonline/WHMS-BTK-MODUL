 ...
 * @version		1.4.0.234
 */
class KerioApi implements KerioApiInterface2 {
	 ...
	 */
		...
		public function confirmConfig($clientTimestampList) {
		

		$method = 'Session.confirmConfig';


		$params = $clientTimestampList ;
		$response = $this->sendRequest($method, $params);
		return $response;
	}
	...
	protected function send($method, $data) {
		...
		/* Open socket */
		$this->socketHandler = new KerioApiSocket2($this->hostname, $this->jsonRpc['port'], $this->timeout);
	...
	//		if ($response['error']) {
				...
				$params = (isset($response['error']['data']))
					...
					: '';
				throw new KerioApiException($message, $code, $params);
		//	}
		}
		elseif (isset($response['result']['errors'])) {
			if ($response['result']['errors'][0]['message']) {
				...
				throw new KerioApiException('hata yeni.'.  $response['result']['errors'][0]['message']);
			}
		}
