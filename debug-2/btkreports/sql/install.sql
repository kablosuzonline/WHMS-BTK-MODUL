-- // --- BÖLÜM 1 / 1 BAŞI - (install.sql - Tam Sürüm)
...
-- Abone Rehber ana verilerini tutacak tablo (BTK formatına uygun, her hizmet için tek kayıt)
CREATE TABLE IF NOT EXISTS `mod_btk_abone_rehber` (
    ...
    `MUSTERI_ID` VARCHAR(50) DEFAULT NULL COMMENT 'WHMCS Client ID olabilir veya özel bir ID',
    `HAT_NO` VARCHAR(50) DEFAULT NULL COMMENT 'WHMCS Service ID veya domain adı olabilir',
    `HAT_DURUM` CHAR(1) DEFAULT NULL COMMENT 'A:Aktif, I:İptal, D:Dondurulmuş, K:Kısıtlı (mod_btk_ref_hat_durum)',
    `HAT_DURUM_KODU` VARCHAR(10) DEFAULT NULL COMMENT 'EK-1 Tablosundaki Kod (mod_btk_ref_hat_durum_kodlari)',
    `HAT_DURUM_KODU_ACIKLAMA` VARCHAR(255) DEFAULT NULL COMMENT 'HAT_DURUM_KODU nun açıklaması, referans tablodan alınır',
    `MUSTERI_HAREKET_KODU` VARCHAR(10) DEFAULT NULL COMMENT 'Son yapılan hareketin kodu (mod_btk_ref_musteri_hareket_kodlari)',
    `MUSTERI_HAREKET_ACIKLAMA` VARCHAR(255) DEFAULT NULL COMMENT 'Son yapılan hareketin açıklaması',
    `MUSTERI_HAREKET_ZAMANI` VARCHAR(14) DEFAULT NULL COMMENT 'Son yapılan hareketin zamanı (YYYYAAGGSSDDSS formatında)',
    `HIZMET_TIPI` VARCHAR(50) DEFAULT NULL COMMENT 'EK-3 Tablosundaki Kod (mod_btk_ref_hizmet_tipleri)',
    `MUSTERI_TIPI` VARCHAR(50) DEFAULT NULL COMMENT 'B-BIREYSEL, G-SIRKET, K-KAMU, D-DIGER (mod_btk_ref_musteri_tipleri)',
    `ABONE_BASLANGIC` VARCHAR(14) DEFAULT NULL COMMENT 'Hizmet başlangıç zamanı (YYYYAAGGSSDDSS formatında)',
    `ABONE_BITIS` VARCHAR(14) DEFAULT NULL COMMENT 'Hizmet bitiş zamanı (YYYYAAGGSSDDSS, aktifse 00000000000000)',
    ...
    `ABONE_CINSIYET` CHAR(1) DEFAULT NULL COMMENT 'E:Erkek, K:Kadın, D:Diğer (mod_btk_ref_cinsiyet)',
    `ABONE_UYRUK` CHAR(3) DEFAULT NULL COMMENT 'ISO 3166-1 Alpha-3 Ülke Kodu (mod_btk_ref_ulkeler)',
    ...
    `ABONE_DOGUM_TARIHI` VARCHAR(8) DEFAULT NULL COMMENT 'YYYYAAGG formatında',
    `ABONE_MESLEK` VARCHAR(10) DEFAULT NULL COMMENT 'EK-6 Meslek Kodu (mod_btk_ref_meslek_kodlari)',
    ...
    `ABONE_KIMLIK_TIPI` CHAR(1) DEFAULT NULL COMMENT 'EK-5 Kimlik Tipi (mod_btk_ref_kimlik_tipleri)',
    `ABONE_KIMLIK_SERI_NO` VARCHAR(20) DEFAULT NULL,
    `ABONE_KIMLIK_VERILDIGI_YER` VARCHAR(100) DEFAULT NULL,
    `ABONE_KIMLIK_VERILDIGI_TARIH` VARCHAR(8) DEFAULT NULL COMMENT 'YYYYAAGG formatında',
    `ABONE_KIMLIK_AIDIYETI` CHAR(1) DEFAULT NULL COMMENT 'EK-5 Kimlik Aidiyeti (mod_btk_ref_kimlik_aidiyeti)',
    ...
    `ABONE_ADRES_YERLESIM_ADRES_KODU` VARCHAR(20) DEFAULT NULL COMMENT 'UAVT Kodu',
...
-- Abone Hareketlerini anlık olarak yakalayıp BTK'ya gönderilmek üzere saklayacak tablo
CREATE TABLE IF NOT EXISTS `mod_btk_abone_hareket_canli` (
    `id` BIGINT AUTO_INCREMENT PRIMARY KEY,
    `abone_rehber_id` BIGINT DEFAULT NULL COMMENT 'mod_btk_abone_rehber.id ile FK (Opsiyonel, veri bütünlüğü için)',
    `whmcs_user_id` INT DEFAULT NULL COMMENT 'tblclients.id',
    `whmcs_service_id` INT DEFAULT NULL COMMENT 'tblhosting.id',
    `OPERATOR_KOD` VARCHAR(10) DEFAULT NULL,
    ...
    `ISS_KULLANICI_ADI` VARCHAR(100) DEFAULT NULL,
    `ISS_POP_BILGISI` VARCHAR(255) DEFAULT NULL,
    `gonderildi_ana_ftp` TINYINT(1) DEFAULT 0 COMMENT '0: Gönderilmedi, 1: Ana FTPye Gönderildi',
    `gonderildi_yedek_ftp` TINYINT(1) DEFAULT 0 COMMENT '0: Gönderilmedi, 1: Yedek FTPye Gönderildi (Eğer aktifse)',
    `gonderildigi_dosya_adi_ana` VARCHAR(255) DEFAULT NULL COMMENT 'Ana FTPye gönderilen dosyanın adı',
    `gonderildigi_dosya_adi_yedek` VARCHAR(255) DEFAULT NULL COMMENT 'Yedek FTPye gönderilen dosyanın adı',
...
CREATE TABLE IF NOT EXISTS `mod_btk_personel` (
    ...
    `firma_unvani` VARCHAR(255) DEFAULT NULL COMMENT 'Operatörün tam ticari unvanı, mod_btk_ayarlar tablosundan çekilebilir',
    `ad` VARCHAR(100) DEFAULT NULL COMMENT 'tbladmins.firstname',
    `soyad` VARCHAR(100) DEFAULT NULL COMMENT 'tbladmins.lastname',
    `tc_kimlik_no` VARCHAR(11) DEFAULT NULL,
    `uyruk_iso_kodu` CHAR(3) DEFAULT 'TUR' COMMENT 'Personelin uyruğu, ISO 3166-1 Alpha-3 (mod_btk_ref_ulkeler)',
    `unvan_gorev` VARCHAR(255) DEFAULT NULL COMMENT 'Personelin şirketteki unvanı/görevi',
    `departman_id` INT DEFAULT NULL COMMENT 'mod_btk_personel_departmanlari.id ile FK',
    `mobil_telefonu` VARCHAR(20) DEFAULT NULL COMMENT 'Alan koduyla birlikte 10 hane',
    `sabit_telefonu` VARCHAR(20) DEFAULT NULL COMMENT 'Alan koduyla birlikte 10 hane',
    `eposta_adresi` VARCHAR(191) DEFAULT NULL COMMENT 'tbladmins.email',
    ...
    `btk_listesine_eklensin` TINYINT(1) DEFAULT 1 COMMENT '0: Hayır (Örn: API Kullanıcısı), 1: Evet (BTK Personel Listesine Dahil Edilecek)',
    `gorev_bolgesi_il_id` INT DEFAULT NULL COMMENT 'Teknik personelin sorumlu olduğu il (mod_btk_adres_il.id)',
    `gorev_bolgesi_ilce_id` INT DEFAULT NULL COMMENT 'Teknik personelin sorumlu olduğu ilçe (mod_btk_adres_ilce.id)',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `mod_btk_gonderilen_dosyalar` (
    `id` BIGINT AUTO_INCREMENT PRIMARY KEY,
    `dosya_adi` VARCHAR(255) NOT NULL COMMENT 'Oluşturulan ve gönderilen dosyanın tam adı (örn: IZMARBILISIM_701_ISS_ABONE_REHBER_20231106115500_01.abn.gz)',
    `dosya_tipi` ENUM('REHBER', 'HAREKET', 'PERSONEL') NOT NULL,
    `ftp_sunucu_tipi` ENUM('ANA', 'YEDEK') NOT NULL DEFAULT 'ANA' COMMENT 'Dosyanın gönderildiği FTP sunucusu',
    `gonderme_zamani` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `cnt_numarasi` VARCHAR(2) NOT NULL DEFAULT '01' COMMENT 'Dosyanın gönderim sayacı (01, 02, ..., 99)',
    `dosya_icerik_hash` VARCHAR(64) DEFAULT NULL COMMENT 'Dosya içeriğinin SHA256 hash değeri (sıkıştırılmamış .abn veya .xlsx içeriği için)',
    `kayit_sayisi` INT DEFAULT 0 COMMENT 'Dosyadaki abone/hareket/personel satır sayısı',
    `ftp_gonderim_durumu` ENUM('BASARILI', 'BASARISIZ', 'BEKLIYOR') DEFAULT 'BEKLIYOR',
    `ftp_hata_mesaji` TEXT DEFAULT NULL,
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY `uidx_dosya_adi_sunucu_tipi` (`dosya_adi`, `ftp_sunucu_tipi`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Adres İl Tablosu (Türkiye için)
CREATE TABLE IF NOT EXISTS `mod_btk_adres_il` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `il_adi` VARCHAR(100) NOT NULL,
    `plaka_kodu` VARCHAR(2) DEFAULT NULL,
    `uavt_kodu` VARCHAR(20) DEFAULT NULL COMMENT 'Ulusal Adres Veri Tabanı İl Kodu',
    UNIQUE KEY `il_adi_unique` (`il_adi`),
    UNIQUE KEY `plaka_kodu_unique` (`plaka_kodu`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Adres İlçe Tablosu (Türkiye için)
CREATE TABLE IF NOT EXISTS `mod_btk_adres_ilce` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `il_id` INT NOT NULL,
    `ilce_adi` VARCHAR(100) NOT NULL,
    `uavt_kodu` VARCHAR(20) DEFAULT NULL COMMENT 'Ulusal Adres Veri Tabanı İlçe Kodu',
...
-- Adres Mahalle Tablosu (Türkiye için)
CREATE TABLE IF NOT EXISTS `mod_btk_adres_mahalle` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `ilce_id` INT NOT NULL,
    `mahalle_adi` VARCHAR(191) NOT NULL,
    `uavt_kodu` VARCHAR(20) DEFAULT NULL COMMENT 'Ulusal Adres Veri Tabanı Mahalle Kodu',
...
CREATE TABLE IF NOT EXISTS `mod_btk_iss_pop_noktalari` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `pop_adi` VARCHAR(255) NOT NULL COMMENT 'POP Noktasının adı/tanımı',
    `il_id` INT DEFAULT NULL COMMENT 'POP Noktasının bulunduğu İl (mod_btk_adres_il.id)',
    `ilce_id` INT DEFAULT NULL COMMENT 'POP Noktasının bulunduğu İlçe (mod_btk_adres_ilce.id)',
    `mahalle_id` INT DEFAULT NULL COMMENT 'POP Noktasının bulunduğu Mahalle (mod_btk_adres_mahalle.id)',
    `adres_detay` TEXT DEFAULT NULL COMMENT 'POP Noktasının açık adresi',
    `koordinatlar` VARCHAR(100) DEFAULT NULL COMMENT 'Enlem,Boylam formatında (Örn: 41.015137,28.979530)',
    `yayin_yapilan_ssid` VARCHAR(191) DEFAULT NULL COMMENT 'Eğer varsa, bu POP üzerinden yayın yapılan SSID adı (ISS_POP_BILGISI için)',
    `sunucu_bilgisi` VARCHAR(255) DEFAULT NULL COMMENT 'İlişkili WHMCS sunucu adı/IP veya POPa özel sunucu bilgisi (ISS_POP_BILGISI için)',
    `aktif_mi` TINYINT(1) DEFAULT 1 COMMENT '0: Pasif, 1: Aktif',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    UNIQUE KEY `pop_adi_unique` (`pop_adi`),
...
CREATE TABLE IF NOT EXISTS `mod_btk_logs` (
    ...
    `islem` VARCHAR(255) DEFAULT NULL COMMENT 'Yapılan işlem, örn: FTP_GONDERIM_ANA, RAPOR_OLUSTURMA_REHBER, TCKN_DOGRULAMA_HATA',
    `mesaj` TEXT DEFAULT NULL,
    `detay` TEXT DEFAULT NULL COMMENT 'Exception mesajı, stack trace veya ek JSON verisi',
    `whmcs_admin_id` INT DEFAULT NULL COMMENT 'İşlemi yapan admin (varsa)',
    `whmcs_user_id` INT DEFAULT NULL COMMENT 'İlgili müşteri (varsa)',
    `whmcs_service_id` INT DEFAULT NULL COMMENT 'İlgili hizmet (varsa)',
...
CREATE TABLE IF NOT EXISTS `mod_btk_product_group_mappings` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `whmcs_product_group_id` INT NOT NULL COMMENT 'tblproductgroups.id',
    `btk_yetki_turu_id` INT NOT NULL COMMENT 'mod_btk_yetki_turleri.id',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- Hizmete özel ek operasyonel bilgileri (BTK dışı, iç kullanım için) tutacak tablo
CREATE TABLE IF NOT EXISTS `mod_btk_hizmet_operasyonel_detaylar` (
    ...
    `mac_adresleri` TEXT DEFAULT NULL COMMENT 'Virgül veya noktalı virgül ile ayrılmış MAC adresleri',
    `cihaz_seri_no` TEXT DEFAULT NULL COMMENT 'Virgül veya noktalı virgül ile ayrılmış cihaz seri numaraları',
    `wifi_sifresi` VARCHAR(255) DEFAULT NULL COMMENT 'Güvenlik nedeniyle uygulama katmanında şifrelenerek saklanmalı',
    ...
    `tesis_koordinatlari` VARCHAR(100) DEFAULT NULL COMMENT 'Enlem,Boylam (Google Maps için, örn: 41.015137,28.979530)',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `mod_btk_logs` (
    ...
    `islem` VARCHAR(255) DEFAULT NULL COMMENT 'Yapılan işlem, örn: FTP_GONDERIM_ANA, RAPOR_OLUSTURMA_REHBER, TCKN_DOGRULAMA_HATA',
    `mesaj` TEXT DEFAULT NULL,
    `detay` TEXT DEFAULT NULL COMMENT 'Exception mesajı, stack trace veya ek JSON verisi', -- BURADA 'detay' OLARAK TANIMLI
    `whmcs_admin_id` INT DEFAULT NULL COMMENT 'İşlemi yapan admin (varsa)',
    `whmcs_user_id` INT DEFAULT NULL COMMENT 'İlgili müşteri (varsa)',
    `whmcs_service_id` INT DEFAULT NULL COMMENT 'İlgili hizmet (varsa)',
    `ip_adresi` VARCHAR(45) DEFAULT NULL
...
-- BTK Referans Tabloları (BTK Teknik Dokümanı EK Tabloları)
...
CREATE TABLE IF NOT EXISTS `mod_btk_ref_hizmet_tipleri` ( `kod` VARCHAR(50) PRIMARY KEY, `aciklama` VARCHAR(255) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT 'BTK EK-3';
CREATE TABLE IF NOT EXISTS `mod_btk_ref_musteri_tipleri` ( `kod` CHAR(1) PRIMARY KEY, `aciklama` VARCHAR(255) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT 'BTK EK-4';
CREATE TABLE IF NOT EXISTS `mod_btk_ref_kimlik_tipleri` ( `kod` CHAR(1) PRIMARY KEY, `aciklama` VARCHAR(255) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT 'BTK EK-5';
CREATE TABLE IF NOT EXISTS `mod_btk_ref_kimlik_aidiyeti` ( `kod` CHAR(1) PRIMARY KEY, `aciklama` VARCHAR(255) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT 'BTK EK-5';
CREATE TABLE IF NOT EXISTS `mod_btk_ref_cinsiyet` ( `kod` CHAR(1) PRIMARY KEY, `aciklama` VARCHAR(50) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT 'BTK Deseninde Geçen Cinsiyet Kodları';
CREATE TABLE IF NOT EXISTS `mod_btk_ref_hat_durum` ( `kod` CHAR(1) PRIMARY KEY, `aciklama` VARCHAR(50) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT 'BTK Deseninde Geçen Genel Hat Durumları';
CREATE TABLE IF NOT EXISTS `mod_btk_ref_meslek_kodlari` ( `kod` VARCHAR(10) PRIMARY KEY, `aciklama` VARCHAR(255) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT 'BTK EK-6';
CREATE TABLE IF NOT EXISTS `mod_btk_ref_ulkeler` ( `iso_kodu` CHAR(3) PRIMARY KEY, `ulke_adi_tr` VARCHAR(100) NOT NULL, `ulke_adi_en` VARCHAR(100) DEFAULT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT 'ISO 3166-1 Alpha-3 Ülke Kodları';

-- // --- BÖLÜM 1 / 1 SONU - (install.sql - Tam Sürüm)