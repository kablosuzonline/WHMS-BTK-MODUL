-- WHMCS BTK Raporlama Modülü - Başlangıç Temel Referans Verileri
-- Sürüm: 20.0.0 (Operasyon PHOENIX)
-- Bu sürüm, BTK dökümanlarında belirtilen TÜM referans tablolarını eksiksiz olarak içerir.

-- Varsayılan Ayarlar
INSERT INTO `mod_btk_settings` (`setting`, `value`) VALUES
('admin_records_per_page', '20'), ('backup_ftp_enabled', '0'), ('backup_ftp_hareket_path', '/HAREKET_YEDEK/'),
('backup_ftp_host', ''), ('backup_ftp_pass', ''), ('backup_ftp_personel_path', '/PERSONEL_YEDEK/'),
('backup_ftp_port', '21'), ('backup_ftp_pasv', '1'), ('backup_ftp_rehber_path', '/REHBER_YEDEK/'),
('backup_ftp_ssl', '0'), ('backup_ftp_user', ''), ('debug_mode', '0'), ('delete_data_on_deactivate', '0'),
('hareket_archive_table_days', '180'), ('hareket_cron_schedule', '0 1 * * *'), ('hareket_live_table_days', '7'),
('log_records_per_page', '50'), ('main_ftp_hareket_path', '/HAREKET/'), ('main_ftp_host', ''),
('main_ftp_pass', ''), ('main_ftp_pasv', '1'), ('main_ftp_personel_path', '/PERSONEL/'),
('main_ftp_port', '21'), ('main_ftp_rehber_path', '/REHBER/'), ('main_ftp_ssl', '0'),
('main_ftp_user', ''), ('nviKpsPass', ''), ('nviKpsUser', ''), ('operator_code', ''), 
('operator_name', ''), ('operator_title', ''), ('personel_cron_schedule', '0 2 1 1,7 *'), 
('pop_monitoring_cron_schedule', '*/5 * * * *'), ('pop_monitoring_enabled', '0'), 
('pop_monitoring_high_latency_threshold_ms', '500'), ('pop_monitoring_timeout_sec', '2'), 
('rehber_cron_schedule', '0 2 1 * *'), ('send_empty_reports', '1')
...
('AIH_B', 'Altyapı İşletmeciliği Hizmeti (B)', 'AIH'), ('AIH_K', 'Altyapı İşletmeciliği Hizmeti (K)', 'AIH'),
('CTH_IMTIYAZ', 'Çeşitli Telekomünikasyon Hizmetleri (İmtiyaz Sözleşmesi)', 'CTH'), ('GMPCS_MT_B', 'GMPCS Mobil Telefon Hizmeti (B)', 'GMPCS'),
('GMPCS_MT_K', 'GMPCS Mobil Telefon Hizmeti (K)', 'GMPCS'), ('GSM_IMTIYAZ', 'GSM (İmtiyaz Sözleşmesi)', 'GSM'),
...
('IMT2000_UMTS_IMTIYAZ', 'IMT-2000/UMTS (İmtiyaz Sözleşmesi)', 'IMT'), ('ISS_B', 'İnternet Servis Sağlayıcılığı (B)', 'ISS'),
('KABLOYAYIN_B', 'Kablolu Yayın Hizmeti (B)', 'KABLOYAYIN'), ('OKTH_K', 'Ortak Kullanımlı Telsiz Hizmeti (K)', 'OKTH'),
('REHBERLIK_K', 'Rehberlik Hizmeti (K)', 'REHBERLIK'), ('STH_B', 'Sabit Telefon Hizmeti (B)', 'STH'),
('STH_K', 'Sabit Telefon Hizmeti (K)', 'STH'), ('SMSH_B', 'Sanal Mobil Şebeke Hizmeti (B)', 'SMSH'),
('SMSH_K', 'Sanal Mobil Şebeke Hizmeti (K)', 'SMSH'), ('UYDUHAB_B', 'Uydu Haberleşme Hizmeti (B)', 'UYDU'),
('UYDUKABLOTV', 'Uydu ve Kablo TV Hizmetleri', 'UYDUKABLOTV'), ('UYDUPLAT_B', 'Uydu Platform Hizmeti (B)', 'UYDUPLAT');

-- Seçili Yetki Türleri (Başlangıçta hepsi pasif)
INSERT IGNORE INTO `mod_btk_secili_yetki_turleri` (`yetki_kodu`, `aktif`) SELECT `yetki_kodu`, 0 FROM `mod_btk_yetki_turleri_referans`;

-- Departmanlar
INSERT IGNORE INTO `mod_btk_personel_departmanlari` (`departman_adi`) VALUES ('Yönetim'), ('Muhasebe'), ('Teknik Destek'), ('Saha Ekibi'), ('Müşteri Hizmetleri');

-- Hat Durum Kodları (Ek-1)
INSERT IGNORE INTO `mod_btk_hat_durum_kodlari_referans` (`kod`, `aciklama`) VALUES ('1','AKTIF'),('5','IPTAL_MUSTERI_TALEBI'),('11','IPTAL_SEHVEN_GIRIS'),('13','KISITLI'),('15','DONDURULMUS');

-- Müşteri Hareket Kodları (Ek-2)
INSERT IGNORE INTO `mod_btk_musteri_hareket_kodlari_referans` (`kod`, `aciklama`) VALUES ('1','HAT DURUM DEGISIKLIGI'),('2','YENI_ABONELIK_KAYDI'),('3','SIM_KART_DEGISIKLIGI'),('4','ODEME_TIPI_DEGISIKLIGI'),('5','ADRES_DEGISIKLIGI'),('6','IMSI_DEGISIKLIGI'),('7','TARIFE_DEGISIKLIGI'),('8','DEVIR_MUSTERI_DEGISIKLIGI'),('9','NUMARA_DEGISIKLIGI'),('10','HAT_IPTAL'),('11','MUSTERI_BILGI_DEGISIKLIGI'),('12','NUMARA_TASIMA'),('13','NUMARA_DEGISMEDEN_NAKIL'),('14','NUMARA_DEGISTIREREK_NAKIL'),('15','IP_DEGISIKLIGI');

-- Hizmet Tipleri (Ek-3) - TAM LİSTE
INSERT IGNORE INTO `mod_btk_ek3_hizmet_tipleri` (`hizmet_tipi_kodu`, `hizmet_tipi_aciklamasi`, `ana_yetki_grup`) VALUES
('ADSL', 'ADSL İnternet Erişimi', 'ISS'), ('XDSL', 'XDSL İnternet Erişimi', 'ISS'), ('FIBER', 'Fiber İnternet Erişimi', 'ISS'), ('HIPER', 'Hiper İnternet Erişimi', 'ISS'), ('FR', 'FR (Frame Relay) Veri İletimi', 'ISS'), ('ATM', 'ATM Veri İletimi', 'ISS'), ('METRO', 'Metro Ethernet İnternet Erişimi', 'ISS'), ('WIFI', 'WiFi İnternet Erişimi', 'ISS'), ('SSG_INTERNET', 'SSG İnternet Erişimi', 'ISS'), ('VERI_MERKEZI', 'Veri Merkezi Hizmeti (ISS kapsamında)', 'ISS'), ('UYDU_INTERNET', 'Uydu Üzerinden İnternet Erişimi', 'ISS'), ('KABLONET', 'Kablo İnternet Erişimi', 'ISS'), ('SAYISAL_COKLU_HAT', 'Sayısal Çoklu Hat (ISDN PRI/BRI)', 'STH'), ('SAYISAL_COKLU_SERVIS_444', 'Sayısal Çoklu Hat Servis 444', 'STH'), ('SAYISAL_COKLU_SERVIS_3RAKAM', 'Sayısal Çoklu Hat Servis 3 Rakam (1XY)', 'STH'), ('SAYISAL_COKLU_SERVIS_822', 'Sayısal Çoklu Hat Servis 822', 'STH'), ('SAYISAL_COKLU_SERVIS_800', 'Sayısal Çoklu Hat Servis 800', 'STH'), ('SANAL_SANTRAL', 'Sanal Santral Hizmeti', 'STH'), ('IP_COKLU_HAT', 'IP Çoklu Hat (SIP Trunk)', 'STH'), ('IP_COKLU_SERVIS_444', 'IP Çoklu Hat Servis 444', 'STH'), ('TELEKS', 'Teleks Hizmeti', 'STH'), ('SABIT_HAT', 'Sabit Telefon Hattı (POTS/PSTN)', 'STH'), ('SERVIS_444_STH', 'Servis 444 (STH)', 'STH'), ('ANKESOR', 'Ankesörlü Telefon Hizmeti', 'STH'), ('MULTIMEDYA_ANKESOR', 'Multimedya Ankesör Hizmeti', 'STH'), ('CENTREKS', 'Centreks Hizmeti', 'STH'), ('SERVIS_3RAKAM_STH', 'Servis 3 Rakam (1XY) (STH)', 'STH'), ('SAYISAL_IKILI_HAT', 'Sayısal İkili Hat (ISDN BRA)', 'STH'), ('SERVIS_800_STH', 'Servis 800 (STH)', 'STH'), ('TELEFON_STH', 'Telefon Hizmeti (Genel Sabit)', 'STH'), ('ISDN_PRI', 'ISDN PRI (STH)', 'STH'), ('WIMAX_STH', 'WIMAX (Sabit Telefon kapsamında)', 'STH'), ('GORUNTULU_TELEFON', 'Görüntülü Telefon (Sabit)', 'STH'), ('KIRALIKDEVRE_TELEFON', 'Kiralık Devre Telefon Hizmeti', 'STH'), ('KIRALIK_DEVRE_TELGRAF', 'Kiralık Devre Telgraf Hizmeti', 'STH'), ('ISDN_BULUT_AKADEMI', 'ISDN Bulut Akademi', 'STH'), ('KURUMSAL_GUVENLIK_STH', 'Kurumsal Güvenlik (Sabit Telefon kapsamında)', 'STH'), ('POSTPAID_GSM', 'Faturalı GSM Hizmeti', 'GSM'), ('PREPAID_GSM', 'Ön Ödemeli GSM Hizmeti', 'GSM'), ('HSCSD', 'HSCSD (Mobil Veri)', 'GSM'), ('GSMPOS', 'GSM POS Hizmeti', 'GSM'), ('PREPAID_INTERNET_MOBIL', 'Ön Ödemeli Mobil İnternet', 'GSM'), ('POSTPAID_INTERNET_MOBIL', 'Faturalı Mobil İnternet', 'GSM'), ('PREPAID_M2M', 'Ön Ödemeli M2M Hattı', 'GSM'), ('POSTPAID_M2M', 'Faturalı M2M Hattı', 'GSM'), ('HT_GSM1800', 'Hava Taşıtlarında GSM1800 Hizmeti', 'GSM'), ('UYDU_SES', 'Uydu Üzerinden Ses Hizmeti', 'UYDU'), ('UYDU_DATA', 'Uydu Üzerinden Data Hizmeti', 'UYDU'), ('TES_UYDU_IP_VE_TELEFON', 'TES-Uydu IP ve Telefon Hizmeti', 'UYDU'), ('P2P_METRO', 'Noktadan Noktaya Metro Ethernet (AIH)', 'AIH'), ('P2P_ATM', 'Noktadan Noktaya ATM (AIH)', 'AIH'), ('P2P_FR', 'Noktadan Noktaya Frame Relay (AIH)', 'AIH'), ('P2P_GHDSL', 'Noktadan Noktaya G.SHDSL (AIH)', 'AIH'), ('P2P_ADSL', 'Noktadan Noktaya ADSL (AIH)', 'AIH'), ('VPN_METRO', 'VPN Metro Ethernet (AIH)', 'AIH'), ('VPN_GHDSL', 'VPN G.SHDSL (AIH)', 'AIH'), ('VPN_ADSL', 'VPN ADSL (AIH)', 'AIH'), ('VPN_FR', 'VPN FR (AIH)', 'AIH'), ('KIRALIK_DEVRE_DATA', 'Kiralık Devre Data (AIH)', 'AIH'), ('KIRALIK_DEVRE_KISMI', 'Kiralık Devre Kısmi (AIH)', 'AIH'), ('VPN_MPLS', 'VPN MPLS (AIH)', 'AIH'), ('P2P_DWDM', 'Noktadan Noktaya DWDM (AIH)', 'AIH'), ('P2P_SDH', 'Noktadan Noktaya SDH (AIH)', 'AIH'), ('SMS_TOPLU_STH', 'SMS / Toplu SMS (PSTN/STH)', 'STH'), ('GMPCS_B', 'GMPCS Mobil Telefon Hizmeti (B) (Ana Yetki)', 'GMPCS'), ('GMPCS_K', 'GMPCS Mobil Telefon Hizmeti (K) (Ana Yetki)', 'GMPCS'), ('IMT_SAY_SIN_KUL_HAK', 'IMT (Sayısı Sınırlandırılmış Kullanım Hakkı Yetki Belgesi) (Ana Yetki)', 'IMT'), ('IMT2000_UMTS_IMT', 'IMT-2000/UMTS (İmtiyaz Sözleşmesi) (Ana Yetki)', 'IMT'), ('KABLOYAYIN_B_ANA', 'Kablolu Yayın Hizmeti (B) (Ana Yetki)', 'KABLOYAYIN'), ('OKTH_K_ANA', 'Ortak Kullanımlı Telsiz Hizmeti (K) (Ana Yetki)', 'OKTH'), ('REHBERLIK_K_ANA', 'Rehberlik Hizmeti (K) (Ana Yetki)', 'REHBERLIK'), ('SMSH_B_ANA', 'Sanal Mobil Şebeke Hizmeti (B) (Ana Yetki)', 'SMSH'), ('SMSH_K_ANA', 'Sanal Mobil Şebeke Hizmeti (K) (Ana Yetki)', 'SMSH'), ('UYDUHAB_B_ANA', 'Uydu Haberleşme Hizmeti (B) (Ana Yetki)', 'UYDU'), ('UYDUKABLOTV_ANA', 'Uydu ve Kablo TV Hizmetleri (Ana Yetki)', 'UYDUKABLOTV'), ('UYDUPLAT_B_ANA', 'Uydu Platform Hizmeti (B) (Ana Yetki)', 'UYDUPLAT'), ('CTH_IMTIYAZ_ANA', 'Çeşitli Telekomünikasyon Hizmetleri (İmtiyaz Sözleşmesi) (Ana Yetki)', 'CTH');

-- Kimlik Tipleri (Ek-4) - TAM LİSTE
...
('722', 'Demirciler, alet yapımcıları ve ilgili işlerde çalışanlar'), ('723', 'Makine bakım ve onarım işlerinde çalışanlar'),
('731', 'El sanatları çalışanları'), ('732', 'Basım ile ilgili işlerde çalışanlar'), ('741', 'Elektrikli ekipman kurulumcuları ve tamircileri'), ('742', 'Elektronik ve telekomünikasyon kurulumcuları ve tamircileri'),
...
('821', 'Montajcılar'), ('831', 'Lokomotif motoru sürücüleri ve ilgili çalışanlar'),
