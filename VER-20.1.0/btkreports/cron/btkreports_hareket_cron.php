<?php
/**
 * WHMCS BTK Raporlama Modülü - Abone HAREKET Raporu Cron Job Dosyası
...
// Örnek: '/home/username/public_html/whmcs/init.php'
...
if (file_exists($whmcsInitPath)) {
    ...
    // WHMCS bulunamazsa, kritik bir hata logla ve çık. Bu, en temel gereksinimdir.
    error_log("[BTK-Entegre] Hareket Cron CRITICAL ERROR: WHMCS init.php not found at: " . $whmcsInitPath);
    exit(1);
}
...
use BtkReports\Manager\AboneManager;
use BtkReports\Manager\LogManager;

LogManager::logAction('Abone Hareket Cron Başladı', 'INFO');
echo "BTK Abone Hareket Cron has started...\n";
...
try {
    ...
    if (empty($aktifGruplar)) {
        LogManager::logAction('Abone Hareket Cron Atlandı', 'UYARI', 'Raporlanacak aktif bir yetki grubu bulunamadı.');
    ...
    // 3. Her aktif yetki grubu için Hareket raporunu oluştur ve gönder
    foreach ($aktifGruplar as $grup) {
        ...
        echo "Processing HAREKET report for group: {$grup}...\n";
        
        // Rapor nesnesini fabrika (factory) aracılığıyla oluştur
        $rapor = BtkRaporFactory::create('HAREKET', $settings, $ftpManager, $grup);
    ...
    LogManager::logAction('Abone Hareket Cron Tamamlandı', 'INFO', $toplamGonderim . ' adet rapor gönderimi denendi.');
    echo "BTK Abone Hareket Cron has finished.\n";
    
} catch (\Exception $e) {
    // Tüm süreçte beklenmedik bir hata olursa logla ve çık
    LogManager::logAction('Abone Hareket Cron Genel Hata', 'KRITIK', $e->getMessage());
    echo "A critical error occurred: " . $e->getMessage() . "\n";
    exit(1);
}

// 4. Raporlama bittikten sonra, eski hareket kayıtlarını arşivle/temizle
try {
    AboneManager::archiveOldHareketler();
    echo "Old hareket records have been archived/purged.\n";
} catch (\Exception $e) {
    // Arşivleme hatası, ana görevin başarısını etkilememeli, sadece loglanmalı
    LogManager::logAction('Hareket Arşivleme Cron Hatası', 'HATA', $e->getMessage());
    echo "An error occurred during log archiving: " . $e->getMessage() . "\n";
}
