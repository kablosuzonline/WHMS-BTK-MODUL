 ...
 * Sürüm: 10.0.7 (Nihai Tarih Formatı Düzeltmesi)
 * @package    WHMCS
...
class BtkHelper
{
    ...
    // ==================================================================
    // == BÖLÜM 2: GÜVENLİ TEMP KLASÖR YÖNETİMİ
    // ==================================================================
    ...
    public static function getModuleTempPath(): string
    {
        ...
        if (file_exists($configFile) && is_readable($configFile)) {
            ...
            if ($configContent) {
                if (preg_match('/\$customadminpath\s*=\s*[\'"]([a-zA-Z0-9_-]+)[\'"];/', $configContent, $matches)) {
                    if (!empty($matches[1])) {
    ...
    // ==================================================================
    // == BÖLÜM 5: HOOK ENJEKSİYON VE İŞLEMCİLER
    // ==================================================================
    ...
    // ==================================================================
    // == BÖLÜM 6: ÇEKİRDEK VERİ YÖNETİMİ (REHBER VE HAREKET)
    // ==================================================================
    ...
    public static function createOrUpdateRehberKayit(int $serviceId, array $guncellenecekVeri = [], ?array $hareketBilgisi = null): void
    {
        try {
            ...
            if ($rehberId && $hareketBilgisi && !empty($hareketBilgisi['kod'])) {
                self::createHareketKaydi($serviceId, $rehberId, $hareketBilgisi['kod'], $hareketBilgisi['aciklama'], $sanitizedData);
            }
        } catch (\Exception $e) {
    ...
    private static function createHareketKaydi(int $serviceId, int $rehberId, string $hareketKodu, string $hareketAciklama, array $snapshotData): void
    {
        ...
        self::logAction('BTK Hareket Kaydı Oluşturuldu', 'INFO', "Hizmet ID: $serviceId, Hareket Kodu: " . $hareketKodu, null, $snapshotData);
    }
    ...
    // ==================================================================
    // == BÖLÜM 7: PERSONEL YÖNETİMİ (GELİŞMİŞ İK)
    // ==================================================================
    ...
    public static function savePersonel(array $data, array $LANG): array
    {
        ...
        $iseBaslama = null;
        if (!empty(trim($data['ise_baslama_tarihi']))) {
            try {
                $dateObj = \DateTime::createFromFormat('Y-m-d', $data['ise_baslama_tarihi']);
                if ($dateObj && $dateObj->format('Y-m-d') === $data['ise_baslama_tarihi']) {
                    $iseBaslama = $data['ise_baslama_tarihi'];
                }
            } catch (\Exception $e) { /* Hatalı formatı null olarak bırak */ }
        }
        
        $istenAyrilma = null;
        if (!empty(trim($data['isten_ayrilma_tarihi']))) {
            try {
                $dateObj = \DateTime::createFromFormat('Y-m-d', $data['isten_ayrilma_tarihi']);
                if ($dateObj && $dateObj->format('Y-m-d') === $data['isten_ayrilma_tarihi']) {
                    $istenAyrilma = $data['isten_ayrilma_tarihi'];
                }
            } catch (\Exception $e) { /* Hatalı formatı null olarak bırak */ }
        }
        ...
        try {
            $dbData = [
                ...
                'acik_adres' => $data['acik_adres'] ?? null,
                'acil_durum_kisi_adi' => $data['acil_durum_kisi_adi'] ?? null,
                'acil_durum_kisi_gsm' => $data['acil_durum_kisi_gsm'] ?? null,
                'btk_listesine_eklensin' => (isset($data['btk_listesine_eklensin']) && $data['btk_listesine_eklensin'] == 1) ? 1 : 0,
            ];

            if ($personelId > 0) {
                Capsule::table('mod_btk_personel')->where('id', $personelId)->update($dbData);
                self::logAction('Personel Güncellendi', 'INFO', "Personel ID: {$personelId} güncellendi.", self::getCurrentAdminId());
                return ['type' => 'success', 'message' => 'Personel bilgileri güncellendi.'];
            } else {
                $newPersonelId = Capsule::table('mod_btk_personel')->insertGetId($dbData);
                self::logAction('Yeni Personel Eklendi', 'INFO', "Yeni personel ID: {$newPersonelId} eklendi.", self::getCurrentAdminId());
    ...
    // ==================================================================
    // == BÖLÜM 8: POP YÖNETİMİ (NETWORK MONITOR)
    // ==================================================================
    ...
    public static function getIssPopNoktalariListWithPagination(array $filters = []): array {
        $query = Capsule::table('mod_btk_iss_pop_noktalari as pop')
            ...
            ->leftJoin('mod_btk_adres_ilce as ilce', 'pop.il_id', '=', 'ilce.id');
    ...
    // ==================================================================
    // == BÖLÜM 9: REFERANS VERİ VE YETKİ FONKSİYONLARI
    // ==================================================================
    ...
    public static function getGrupByYetkiKodu(string $yetkiKodu): ?string {
        $yetki = Capsule::table('mod_btk_yetki_turleri_referans')->where('yetki_kodu', $yetkiKodu)->first();
        return $yetki ? $yetki->grup : null;
    }
    ...
    public static function getAllYetkiTurleriWithStatus(array $formData = null): array {
        $yetkiler = self::arrayFromCollection(Capsule::table('mod_btk_yetki_turleri_referans as ytr')
            ...
            ->orderBy('ytr.yetki_adi')->get());

        if (isset($formData['yetkiler']) && is_array($formData['yetkiler'])) {
            foreach ($yetkiler as $index => $yt) {
                $yetkiler[$index]['aktif'] = array_key_exists($yt['yetki_kodu'], $formData['yetkiler']) ? 1 : 0;
            }
        }
        return $yetkiler;
    }
    ...
    public static function saveSeciliYetkiTurleri(array $postData = []): void
    {
        try {
            ...
            foreach ($tumYetkiKodlari as $kod) {
                $durum = array_key_exists($kod, $secilenYetkiler) ? 1 : 0;
                Capsule::table('mod_btk_secili_yetki_turleri')->updateOrInsert(
                    ['yetki_kodu' => $kod],
                    ['aktif' => $durum]
                );
            }
    ...
    // ==================================================================
    // == BÖLÜM 11: RAPORLAMA YARDIMCILARI VE DASHBOARD
    // ==================================================================
