...
class AboneHareketRapor extends AbstractBtkRapor
{
    ...
    public function olusturVeGonder(string $ftpType = 'main'): array
    {
        ...
        // Sadece ANA FTP'ye başarılı ve atlanmamış bir gönderim yapıldığında hareketleri işaretle.
    ...
    protected function getReportData(): array
    {
        ...
        $hareketler = Capsule::table('mod_btk_abone_hareket_live as ahl')
            ...
            ->select('ahl.*') // Sadece hareket tablosundaki verileri alıyoruz
            ->get()
            ->map(fn($item) => (array)$item) // Sonucu array'e çevir
            ->all();

        $reportData = [];
        $this->gonderilecekHareketIdleri = [];

        foreach ($hareketler as $hareketData) {
            ...
            if (is_array($snapshotData)) {
                ...
                $this->gonderilecekHareketIdleri[] = $hareketData['id'];
                 ...
                 BtkHelper::logAction('Hareket Veri Hatası', 'HATA', "Hareket ID: {$hareketData['id']} için JSON snapshot verisi okunamadı veya geçersiz.", null, ['json_content' => $hareketData['islem_detayi_json']]);
            }
        }
