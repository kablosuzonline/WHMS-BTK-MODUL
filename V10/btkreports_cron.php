 ...
 * Sürüm: 8.2.0 (Operasyon Merkezi) - Modern, nesne yönelimli ve fabrikasyon tabanlı yapı.
 *
 * @package    WHMCS
...
BtkHelper::logAction('BTK Raporlama Cron v8.2.0 Başladı', 'INFO');
echo "BTK Reporting Cron v8.2.0 has started...\n";
...
try {
    ...
    foreach (['REHBER', 'HAREKET'] as $raporTuru) {
        ...
        BtkHelper::logAction("{$raporTuru} Cron Tetiklendi", 'INFO', "Zamanlama: {$schedule}");
        ...
        foreach ($aktifYetkiGruplari as $grup) {
            try {
                ...
                // Raporu oluştur ve ANA FTP'ye gönder
                $sonucAna = $rapor->olusturVeGonder('main');
                if ($sonucAna['status']) {
                    BtkHelper::logAction("{$raporTuru} Raporu Başarılı (Ana FTP - {$grup})", 'BAŞARILI', $sonucAna['message']);
                } else {
                    BtkHelper::logAction("{$raporTuru} Raporu Hatalı (Ana FTP - {$grup})", 'HATA', $sonucAna['message']);
                }

                // Yedek FTP aktifse, oraya da gönder
                if (($settings['backup_ftp_enabled'] ?? '0') === '1') {
                    $sonucYedek = $rapor->olusturVeGonder('backup');
                    if ($sonucYedek['status']) {
                        BtkHelper::logAction("{$raporTuru} Raporu Başarılı (Yedek FTP - {$grup})", 'BAŞARILI', $sonucYedek['message']);
                    } else {
                        BtkHelper::logAction("{$raporTuru} Raporu Hatalı (Yedek FTP - {$grup})", 'HATA', $sonucYedek['message']);
                    }
                }
    ...
    if (CronExpression::isValidExpression($personelSchedule) && CronExpression::factory($personelSchedule)->isDue($now)) {
        BtkHelper::logAction("PERSONEL Cron Tetiklendi", 'INFO', "Zamanlama: {$personelSchedule}");
        try {
            $rapor = BtkRaporFactory::create('PERSONEL', $settings, $ftpManager);
            
            $sonucAna = $rapor->olusturVeGonder('main');
            if ($sonucAna['status']) {
                BtkHelper::logAction("PERSONEL Raporu Başarılı (Ana FTP)", 'BAŞARILI', $sonucAna['message']);
            } else {
                BtkHelper::logAction("PERSONEL Raporu Hatalı (Ana FTP)", 'HATA', $sonucAna['message']);
            }
            
            if (($settings['backup_ftp_enabled'] ?? '0') === '1') {
                $sonucYedek = $rapor->olusturVeGonder('backup');
                if ($sonucYedek['status']) {
                    BtkHelper::logAction("PERSONEL Raporu Başarılı (Yedek FTP)", 'BAŞARILI', $sonucYedek['message']);
                } else {
                    BtkHelper::logAction("PERSONEL Raporu Hatalı (Yedek FTP)", 'HATA', $sonucYedek['message']);
                }
            }
...
BtkHelper::logAction('BTK Raporlama Cron v8.2.0 Tamamlandı', 'INFO');
echo "BTK Reporting Cron v8.2.0 has completed.\n";
