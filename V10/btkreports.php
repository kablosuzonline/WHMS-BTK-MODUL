 ...
 * Sürüm: 10.0.1 (Gerçek Hibrit Mimarisi)
 * v6'nın fonksiyonel yapısı, v8.5'in modern iskeleti üzerine inşa edilmiştir.
 * @package    WHMCS
...
function btkreports_activate() {
    try {
        ...
        BtkHelper::logAction('Modül Aktivasyonu', 'BAŞARILI', 'BTK Raporlama Modülü v10.0.1 başarıyla etkinleştirildi.');
...
function btk_page_config(&$smarty, $LANG) {
    ...
    $formData = $_SESSION['BtkReportsFormErrorData'] ?? null;
    if ($formData) {
    ...
    $smarty->assign('settings', $settings);
    $smarty->assign('yetki_turleri', BtkHelper::getAllYetkiTurleriWithStatus($formData));
}
...
function btk_action_save_config($postData, $LANG) {
    ...
    BtkHelper::saveSeciliYetkiTurleri($postData);
...
function btk_action_save_single_personel($postData, $LANG) {
    ...
    $data['btk_listesine_eklensin'] = (isset($data['btk_listesine_eklensin']) && ($data['btk_listesine_eklensin'] === 'on' || $data['btk_listesine_eklensin'] === '1')) ? 1 : 0;
...
function btk_action_save_pop_definition($postData, $LANG) {
    ...
    $data['aktif_pasif_durum'] = (isset($data['aktif_pasif_durum']) && ($data['aktif_pasif_durum'] === 'on' || $data['aktif_pasif_durum'] === '1')) ? 1 : 0;
    $data['izleme_aktif'] = (isset($data['izleme_aktif']) && ($data['izleme_aktif'] === 'on' || $data['izleme_aktif'] === '1')) ? 1 : 0;
