 ...
 * Sürüm: 8.5.4 (Anlık Tepkime) - Tüm arayüz hataları giderildi.
 */
...
(function($, window, document, undefined) {
    ...
    var btkModule = {
        ...
        init: function() {
            ...
            console.log('BTK Raporlama Modülü Scriptleri v8.5.4 başarıyla yüklendi.');
        },
        ...
        bindEvents: function() {
            ...
            doc.on('change', '.adres-ilce-select', function() {
                ...
                $this.data('preselect-mahalle-id', '');
            });
                        ...
                        for (var key in p) {
                            ...
                            if ($input.length) {
                                if ($input.is(':checkbox')) {
                                    if (p[key] == 1) { $input.bootstrapToggle('on'); } else { $input.bootstrapToggle('off'); }
                                } else {
                                    $input.val(p[key]);
                                }
                            }
        ...
        handleReportGeneration: function($form, $submitButton) {
            ...
            postData += '&' + $submitButton.attr('name') + '=' + $submitButton.val();
            ...
            $.post($form.attr('action'), postData, function(response) {
                ...
                if (response.file_name && response.status) {
        ...
        validateNviForPersonel: function($button) {
            ...
            var tckn = $modal.find('#modal_tckn').val();
            var ad = $modal.find('#modal_ad').val();
            var soyad = $modal.find('#modal_soyad').val();
            var dogumTarihi = $modal.find('#modal_dogum_tarihi').val();
            var dogumYili = dogumTarihi ? dogumTarihi.substring(0, 4) : '';
            var $statusDiv = $modal.find('.tckn-validation-status-modal');

            if (!tckn || !ad || !soyad || !dogumYili) {
                $statusDiv.html('<span class="text-danger">TCKN, Ad, Soyad ve Doğum Tarihi alanları zorunludur.</span>');
                return;
            }
