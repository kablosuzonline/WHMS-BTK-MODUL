-- // --- BÖLÜM 1 / 1 BAŞI - (install.sql - Tam Sürüm - Foreign Key Sıralaması Düzeltilmiş)
...
-- Tablo oluşturma sıralaması Foreign Key bağımlılıklarına göre düzenlenmiştir.

-- 1. Bağımsız Tablolar veya En Az Bağımlı Olanlar
...
-- BTK Referans Tabloları (Bağımsızlar)
...
CREATE TABLE IF NOT EXISTS `mod_btk_gonderilen_dosyalar` (
    ...
    `dosya_adi` VARCHAR(255) NOT NULL COMMENT 'Oluşturulan ve gönderilen dosyanın tam adı',
...
CREATE TABLE IF NOT EXISTS `mod_btk_logs` (
    ...
    `islem` VARCHAR(255) DEFAULT NULL COMMENT 'Yapılan işlem',
...
-- 2. Birinci Seviye Bağımlı Tablolar
...
-- 3. İkinci Seviye Bağımlı Tablolar
...
CREATE TABLE IF NOT EXISTS `mod_btk_personel` (
    ...
    `firma_unvani` VARCHAR(255) DEFAULT NULL COMMENT 'Operatörün tam ticari unvanı',
    ...
    `uyruk_iso_kodu` CHAR(3) DEFAULT 'TUR' COMMENT 'Personelin uyruğu, (mod_btk_ref_ulkeler)',
    ...
    `btk_listesine_eklensin` TINYINT(1) DEFAULT 1 COMMENT '0: Hayır, 1: Evet (BTK Personel Listesine Dahil Edilecek)',
...
CREATE TABLE IF NOT EXISTS `mod_btk_abone_rehber` (
    ...
    `MUSTERI_HAREKET_ZAMANI` VARCHAR(14) DEFAULT NULL COMMENT 'Son yapılan hareketin zamanı (YYYYAAGGSSDDSS)',
    ...
    `ABONE_BASLANGIC` VARCHAR(14) DEFAULT NULL COMMENT 'Hizmet başlangıç zamanı (YYYYAAGGSSDDSS)',
...
-- 4. Üçüncü Seviye Bağımlı Tablolar
CREATE TABLE IF NOT EXISTS `mod_btk_iss_pop_noktalari` (
    ...
    `koordinatlar` VARCHAR(100) DEFAULT NULL COMMENT 'Enlem,Boylam formatında',
    `yayin_yapilan_ssid` VARCHAR(191) DEFAULT NULL COMMENT 'Bu POP üzerinden yayın yapılan SSID adı',
    `sunucu_bilgisi` VARCHAR(255) DEFAULT NULL COMMENT 'İlişkili WHMCS sunucu adı/IP veya POPa özel sunucu bilgisi',
...
CREATE TABLE IF NOT EXISTS `mod_btk_abone_hareket_canli` (
    `id` BIGINT AUTO_INCREMENT PRIMARY KEY,
    `abone_rehber_id` BIGINT DEFAULT NULL COMMENT 'mod_btk_abone_rehber.id ile FK (Opsiyonel)',
    `whmcs_user_id` INT DEFAULT NULL COMMENT 'tblclients.id',
...
-- 5. Dördüncü Seviye Bağımlı Tablolar (veya diğerleri)
...
CREATE TABLE IF NOT EXISTS `mod_btk_hizmet_operasyonel_detaylar` (
    ...
    `wifi_sifresi` VARCHAR(255) DEFAULT NULL COMMENT 'Uygulama katmanında şifrelenerek saklanmalı',
    ...
    `tesis_koordinatlari` VARCHAR(100) DEFAULT NULL COMMENT 'Enlem,Boylam (örn: 41.015137,28.979530)',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY `hizmet_id_unique` (`hizmet_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- // --- BÖLÜM 1 / 1 SONU - (install.sql - Tam Sürüm - Foreign Key Sıralaması Düzeltilmiş)