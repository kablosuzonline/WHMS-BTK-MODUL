{*
    WHMCS BT (Operatör)** için, yöneticinin manuel olarak rapor oluşturmasını, indirmesini ve FTP sunucularına göndK Raporlama Modülü - Manuel Rapor Oluşturma ve Gönderme Sayfası
    Sürermesini sağlayan, dinamik AJAX sonuç gösterimini içeren, üretime hazır ve **Tam Sürüm** `üm: 7.2 (Operatör)
*}

{if $flashMessage}
    <div class="alertgenerate_reports.tpl` dosyası.

Bu şablon, her rapor türü için ayrı bir akordiyon alert-{$flashMessage.type|default:'info'} text-center" role="alert">
        <i menü elemanı sunarak arayüzü temiz ve kullanışlı tutar.

---

**DOSYA: class="fas {if $flashMessage.type == 'success'}fa-check-circle{elseif $flashMessage.type == 'error'}fa-exclamation-circle{else}fa-info-circle{/if}"> `templates/admin/generate_reports.tpl` (TAM SÜRÜM - NİHAİ)**

```html
{*
    WHMCS BTK Raporlama Modülü - Manuel Rapor Oluşturma ve Gö</i>
        {$flashMessage.message}
    </div>
{/if}

<p class="lead">{$LANG.generateReportsIntro}</p>

<div class="panel-group" id="accordionGenerate" role="tablist"nderme Sayfası
    Sürüm: 7.2 (Operatör)
*}
        ...
        <div class="panel-heading" role="tab" id="headingRehber">
            <h4 class="panel-title">
                <a role="button" data-toggle="collapse" data-parent="#accordionGenerate" href="#collapseRehber" aria-expanded="true" aria-controls="collapseRehber">
                    <i class="fas fa-address-book"></i>="btk-report-global-flash-message">
{if $flashMessage}
...
<div class="panel-group" id="accordionGenerate" role="tablist" aria-multiselectable="true">
    ...
    {* ABONE REHBER RAPORU *}
    <div class {$LANG.generateRehberReportTitle}
                </a>
            </h4>
        </div>
        <div id="collapseRehber" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingRehber">
            <div class="panel-body">
                <p class="help-block">{$LANG.generateRehberReportHelpText|default:'Seçili Yetki Türü için tüm abonelerin güncel bilgilerini içeren REHBER raporunu oluşturur.'}</p>
                <form method="post" action="{$modulelink}&action=do_generate_report" class="form-inline report-generation-form">
                    ...
                    <div class="form-group">
                        ...
                        <select name="yetki_tipi_kodu" id="rehber_yetki_tipi" class="panel panel-default btk-widget">
        <div class="panel-heading" role="tab" id="headingRehber">
            <h4 class="panel-title">
                <a role="button" data-toggle="collapse" data-parent="#accordionGenerate" href="#collapseRehber" aria-expanded="true" aria-controls="collapseRehber">
        ...
        <div id="collapseRehber" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingRehber">
            <div class="panel-body">
                ...
                <form method="post" action="{$modulelink}&action=do_generate_report" class="form-inline report-generation-form">
                    ...
                    <div class="form-group">
                        ...
                        <select name="yetki_tipi_kodu" id="rehber_yetki_tipi" class="form-control" required>
                            ...
                            {foreach from=$activeBtkAuthTypesForReports item=authType}
                                <option value="{$authType="form-control" required>
                            <option value="">{$LANG.selectOption}</option>
                            {foreach from=$activeBtkAuthTypesForReports item=authType}
                    ...
                    <button type="submit" name="generate_only" class="btn btn-info"><i class="fas fa-cogs"></i> {$LANG.generateOnlyButton}</button>
                    <button type="submit" name="send_main" class="btn btn-primary"><i class="fas fa-upload"></i> {$LANG.generateAndSendMainFtpButton}</button>
                    {if $settings.backup_ftp_enabled == '1'}
                    <button type="submit" name="send_backup" class="btn btn-warning"><i class="fas fa-archive"></i> {$LANG.generateAndSendBackupFtpButton}</button>
                    {/if}
                </form>
    ...
    <div class="panel panel-default btk-widget">
        ...
        <div id="collapseHareket" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingHareket">
            <div class="panel-body">
                <p class="help-block">{$LANG.generateHareketReportHelpText|default:'Son gönderimden bu yana oluşan tüm abone hareketlerini içeren HAREKET raporunu oluşturur.'}</p>
                <form method="post" action="{$modulelink}&action=do_generate_report" class="form-inline report-generation-form">
                    <input type="hidden" name="token" value="{$csrfToken}">
                    <input type="hidden" name="report_type" value="HAREKET">
                     <div class="form-group">
                        <label for="hareket_yetki_tipi">{$LANG.selectBtkAuthTypeForReportLabel}:</label>
                        <select name="yetki_tipi_k->yetki_kodu}">{$authType->yetki_adi} ({$authType->grup|default:$authType->yetki_kodu})</option>
                            {/foreach}
                        </select>
                    </div>
                    <button type="submit" name="generate_only" class="btn btn-info"><i class="fas fa-cogs"></i> {$LANG.generateOnlyButton}</button>
    ...
    <div class="panel panel-default btk-widget">
        ...
        <odu" id="hareket_yetki_tipi" class="form-control" required>
                            <option value="">{$LANG.selectOption}</option>
                            {foreach from=$activeBtkAuthTypesForReports item=authType}
                                <option value="{$authType->yetki_kodu}">{$authType->yetki_adi} ({$authType->grup|default:$authType->yetki_kodu})</option>
                            {/foreach}
