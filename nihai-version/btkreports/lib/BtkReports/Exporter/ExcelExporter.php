...
// Composer autoload dosyası, bu sınıfı çağıran yerlerde zaten dahil edilmiş olmalıdır.
if (!class_exists('PhpOffice\PhpSpreadsheet\Spreadsheet')) {
    // Bu sadece bir güvenlik önlemi, normalde bu hatanın oluşmaması gerekir.
    throw new \Exception("PhpSpreadsheet kütüphanesi yüklenemedi. Lütfen 'composer install' komutunu çalıştırın.");
}
...
class ExcelExporter {
     ...
     * @param string $filename İndirilecek veya kaydedilecek dosyanın tam adı (uzantı dahil).
    ...
    public static function exportPersonelList(array $personelData, string $filename, bool $outputToBrowser = true, ?string $savePath = null): string|bool
    {
        try {
            ...
            if ($outputToBrowser) {
                // Tarayıcıya indirme olarak gönder
                if(ob_get_level()) ob_end_clean(); // Olası çıktı tamponlarını temizle
            ...
            BtkHelper::logAction('Excel Export Hatası (Personel)', 'HATA', 'Personel listesi Excel\'e aktarılırken hata: ' . $e->getMessage());
            return false;
        }
    }
}
