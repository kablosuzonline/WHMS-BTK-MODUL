 ...
 * Sürüm: 7.2.5 (Operatör - Kararlılık ve Hata Düzeltme Sürümü)
 * @package    WHMCS
...
class BtkHelper
{
    ...
    private static function buildPagination(array $filters, int $total, int $limit, int $page): string
    {
        ...
        $html .= ($page > 1) ? '<li><a href="?'.$baseUrl.($page - 1).'">«</a></li>' : '<li class="disabled"><span>«</span></li>';
        ...
        $html .= ($page < $totalPages) ? '<li><a href="?'.$baseUrl.($page + 1).'">»</a></li>' : '<li class="disabled"><span>»</span></li>';
        $html .= '</ul></div>';
        return $html;
    }
    // ==================================================================
    ...
    public static function injectClientProfileForm(array $vars): string
    {
        ...
        $smarty->assign('kimlikTipleri', Capsule::table('mod_btk_kimlik_tipleri_referans')->orderBy('aciklama')->get());
        $smarty->assign('meslekKodlari', Capsule::table('mod_btk_meslek_kodlari_referans')->orderBy('aciklama')->get());
    ...
    {
        if (empty($btkData) || !isset($btkData['tesis_il_id'])) {
            return true;
        }
    ...
    public static function handleServiceDetailsSave(int $serviceId, array $btkData): void
    {
        if (isset($btkData['tesis_adresi_yerlesimle_ayni']) && ($btkData['tesis_adresi_yerlesimle_ayni'] == 'on' || $btkData['tesis_adresi_yerlesimle_ayni'] == '1')) {
            ...
            if ($rehberKayit) {
                $yerlesimData = (array) Capsule::table('mod_btk_abone_rehber')->where('whmcs_client_id', $rehberKayit['whmcs_client_id'])->orderBy('id','desc')->first();
    ...
    public static function handleServiceEdit(array $vars): void
    {
        ...
        // WHMCS'in AdminServiceEdit hook'u eski paket ID'sini vermez, bu yüzden
        // mevcut kayıttan kontrol ediyoruz.
        if ($rehberKayit['packageid'] != $vars['packageid']) {
            self::createOrUpdateRehberKayit($serviceId, ['packageid' => $vars['packageid']], [
    ...
    private static function createHareketKaydi(int $serviceId, int $rehberId, string $hareketKodu, string $hareketAciklama, array $snapshotData): void
    {
        ...
        Capsule::table('mod_btk_abone_hareket_live')->insert([
            ...
            'islem_detayi_json' => json_encode($snapshotData),
            'gonderildi_flag' => 0,
    ...
    {
        $admins = Capsule::table('tbladmins')->get(['id', 'username', 'firstname', 'lastname', 'email']);
        foreach ($admins as $admin) {
            self::syncAdminToPersonel($admin->id, (array)$admin);
        }
    }
    ...
    public static function getPersonelListWithPagination(array $filters = []): array
    {
        ...
        return [
            'data' => $data,
            'pagination' => self::buildPagination($filters, $total, $limit, $page)
        ];
    }
    ...
    public static function getIller() { return Capsule::table('mod_btk_adres_il')->orderBy('il_adi')->get()->all(); }
    public static function getIlcelerByIlId(int $ilId) { if ($ilId <= 0) return []; return Capsule::table('mod_btk_adres_ilce')->where('il_id', $ilId)->orderBy('ilce_adi')->get()->all(); }
    public static function getMahallelerByIlceId(int $ilceId) { if ($ilceId <= 0) return []; return Capsule::table('mod_btk_adres_mahalle')->where('ilce_id', $ilceId)->orderBy('mahalle_adi')->get()->all(); }
    public static function getDepartmanList() { return Capsule::table('mod_btk_personel_departmanlari')->orderBy('departman_adi')->get()->all(); }
    
    public static function getAllEk3HizmetTipleri(?string $grup = null) { 
        $query = Capsule::table('mod_btk_ek3_hizmet_tipleri');
        if($grup) $query->where('ana_yetki_grup', $grup);
        return $query->orderBy('hizmet_tipi_aciklamasi')->get()->all();
    }
    
    public static function getBtkYetkiForProductGroup(int $gid) {
        return (array) Capsule::table('mod_btk_product_group_mappings as pgm')
    ...
    public static function getAktifYetkiTurleri(string $returnAs = 'object_list') {
        ...
        return $query->select('syt.yetki_kodu', 'ytr.yetki_adi', 'ytr.grup')->get()->all();
    }
    ...
    public static function getAllYetkiTurleriWithStatus(): array
    {
        return Capsule::table('mod_btk_yetki_turleri_referans as ytr')
            ...
            ->select('ytr.yetki_kodu', 'ytr.yetki_adi', 'ytr.grup', 'syt.aktif')
    ...
    {
        return Capsule::table('mod_btk_product_group_mappings')->pluck('ek3_hizmet_tipi_kodu', 'gid')->all();
    }
    ...
    public static function getLogsWithPagination(array $filters = []): array
    {
        ...
        $generalData = $generalQuery->orderBy('id', 'desc')->skip($offsetGeneral)->take($limit)->get()->all();

        // FTP Logs
        $totalFtp = $ftpQuery->count();
        $pageFtp = (int)($filters['page_ftp'] ?? 1);
        $offsetFtp = ($pageFtp - 1) * $limit;
        $ftpData = $ftpQuery->orderBy('id', 'desc')->skip($offsetFtp)->take($limit)->get()->all();
        
        return [
            'general' => ['data' => $generalData, 'pagination' => self::buildPagination(['page_ftp' => $pageFtp], $totalGeneral, $limit, $pageGeneral)],
            'ftp' => ['data' => $ftpData, 'pagination' => self::buildPagination(['page_general' => $pageGeneral], $totalFtp, $limit, $pageFtp)]
        ];
    }
    ...
    public static function getBtkAlanSiralamasi(string $raporTuru = 'REHBER', ?string $yetkiTuruGrup = null): array
    {
        $ortakAlanlar = [
            ...
            'TESIS_IL', 'TESIS_ILCE', 'TESIS_MAHALLE', 'TESIS_CADDE', 'TESIS_DIS_KAPI_NO', 'TESIS_IC_KAPI_NO',
