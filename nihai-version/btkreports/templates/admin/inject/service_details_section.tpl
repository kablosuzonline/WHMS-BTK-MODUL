    ...
    <table class="form" width="100%" border="0" cellspacing="2" cellpadding="3">
        <tbody>
            ...
            <tr>
                ...
                <td class="fieldarea">
                    <input type="text" name="btk_service_data[hat_no]" value="{$btkServiceData.hat_no|default:$domain|default:$dedicatedip|escape}" class="form-control input-sm" required>
                    <small class="text-muted">{$LANG.serviceHatNoTooltip|default:'BTK\'ya bildirilecek tekil hizmet numarası (Telefon, Devre No, Kullanıcı Adı vb.).'}</small>
                </td>
            </tr>
            <tr>
                ...
                <td class="fieldarea">
                    <select name="btk_service_data[hizmet_tipi]" class="form-control select-inline" required>
                        ...
                        {if $uygunHizmetTipleri}
                            ...
                            {/foreach}
                        {else}
                            <option value="" disabled>{$LANG.noMatchingServiceType|default:'Bu ürün grubuna uygun hizmet tipi bulunamadı. Lütfen Ürün Grubu Eşleştirmesini kontrol edin.'}</option>
                        {/if}
                    </select>
                </td>
            </tr>
            <tr>
                ...
                <td class="fieldarea">
                    <input type="text" name="btk_service_data[abone_baslangic]" value="{$btkServiceData.abone_baslangic|default:$regdate|date_format:"%Y%m%d%H%M%S"|escape}" class="form-control input-sm" placeholder="YYYYMMDDHHMMSS">
                    <small class="text-muted">(Format: YYYYAAGGSSddss UTC)</small>
                </td>
            </tr>
            <tr>
                <td class="fieldlabel">{$LANG.serviceAboneTarifeLabel}</td>
                <td class="fieldarea"><input type="text" name="btk_service_data[abone_tarife]" value="{$btkServiceData.abone_tarife|default:$package.name|escape}" class="form-control input-sm"></td>
            </tr>
            <tr>
                <td class="fieldlabel">{$LANG.serviceStatikIpLabel}</td>
                <td class="fieldarea"><input type="text" name="btk_service_data[statik_ip]" value="{$btkServiceData.statik_ip|default:$dedicatedip|escape}" class="form-control input-sm"></td>
            </tr>
            ...
            {if $mapping.grup == 'ISS'}
                <tr>
                    ...
                    <td class="fieldarea"><input type="text" name="btk_service_data[iss_hiz_profili]" value="{$btkServiceData.iss_hiz_profili|escape}" class="form-control input-sm" placeholder="Örn: 100 Mbps'e kadar"></td>
                </tr>
                <tr>
                    <td class="fieldlabel">{$LANG.issKullaniciAdiLabel}</td>
                    <td class="fieldarea"><input type="text" name="btk_service_data[iss_kullanici_adi]" value="{$btkServiceData.iss_kullanici_adi|default:$username|escape}" class="form-control input-sm"></td>
                </tr>
                 <tr>
            ...
            <tr>
                ...
                <td class="fieldarea">
                    <label class="checkbox-inline">
                        <input type="checkbox" name="btk_service_data[tesis_adresi_yerlesimle_ayni]" id="tesis_adresi_yerlesimle_ayni" {if $tesisAdresiYerlesimleAyni}checked{/if}>
