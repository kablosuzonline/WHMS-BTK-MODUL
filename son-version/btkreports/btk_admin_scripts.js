...
(function($, window, document, undefined) {
    ...
    var btkModule = {
        ...
        initDynamicAddressDropdowns: function(containerSelector) {
            ...
            $container.off('change.btkadres.il').on('change.btkadres.il', '.adres-il-select', function() {
                var $ilSelect = $(this);
                // Önemli: preselect-ilce-id'yi her tetiklemede yeniden oku
                ...
                // data attribute'i bir kez kullanıldıktan sonra temizlenebilir.
                $ilSelect.data('preselect-ilce-id', null); 
            });
            ...
            $container.off('change.btkadres.ilce').on('change.btkadres.ilce', '.adres-ilce-select', function() {
                var $ilceSelect = $(this);
                // Önemli: preselect-mahalle-id'yi her tetiklemede yeniden oku
                ...
                // data attribute'i bir kez kullanıldıktan sonra temizlenebilir.
                $ilceSelect.data('preselect-mahalle-id', null); 
            });
        },
        ...
        initConfigPage: function() {
            ...
            // Bootstrap Toggle'ın DOM'u manipüle etmesinden sonra çalışır.
                    ...
                    $.post(btkModule.vars.moduleLink, postData, function(data) {
                        if (data && data.status === 'success' && data.data) {
                            ...
                            if (p.il_id) {
                                ...
                                // data-preselect-ilce-id ve data-preselect-mahalle-id'yi doğrudan select elementlerine bağla
        ...
        initInjectedForms: function() {
            ...
            if ($('#btkServiceDetailsPlaceholder').length) {
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="fieldarea" id="tesisAdresiFormAlani" style="display:${btkData.tesisAdresiYerlesimleAyni ? 'none' : 'block'};">
                                        <div class="row">
                                            <div class="col-sm-4"><select name="btk_service_data[tesis_il_id]" id="tesis_il_id" class="form-control input-sm adres-il-select" data-target-ilce="tesis_ilce_id" data-preselect-ilce-id="${btkData.tesis_ilce_id || ''}"><option value="">${lang.selectIlOption}</option>${(btkData.iller || []).map(il => `<option value="${il.id}" ${btkData.tesis_il_id == il.id ? 'selected' : ''}>${btkModule.escapeHtml(il.il_adi)}</option>`).join('')}</select></div>
                                            <div class="col-sm-4"><select name="btk_service_data[tesis_ilce_id]" id="tesis_ilce_id" class="form-control input-sm adres-ilce-select" data-target-mahalle="tesis_mahalle_id" data-preselect-mahalle-id="${btkData.tesis_mahalle_id || ''}"><option value="">${lang.selectIlceOption}</option>${(btkData.tesis_ilceler || []).map(ilce => `<option value="${ilce.id}" ${btkData.tesis_ilce_id == ilce.id ? 'selected' : ''}>${btkModule.escapeHtml(ilce.ilce_adi)}</option>`).join('')}</select></div>
                                            <div class="col-sm-4"><select name="btk_service_data[tesis_mahalle_id]" id="tesis_mahalle_id" class="form-control input-sm adres-mahalle-select"><option value="">${lang.selectMahalleOption}</option>${(btkData.tesis_mahalleler || []).map(mahalle => `<option value="${mahalle.id}" ${btkData.tesis_mahalle_id == mahalle.id ? 'selected' : ''}>${btkModule.escapeHtml(mahalle.mahalle_adi)}</option>`).join('')}</select></div>
                                        </div>
                ...
                `;
                // Nereye enjekte edeceğiz? Genellikle "#tab-details" veya benzeri bir div içine.
                // WHMCS 7.x ve 8.x'de hizmet detaylarında module commands divi altında bir yer bulabiliriz.
                // En güvenlisi, sayfanın sonuna eklemek.
                $('.tab-content #tabdetails').append(htmlContent); // Example: Add to a specific tab
                // Alternatif olarak, sayfanın sonunda boş bir placeholder div bıraktık.
                // $placeholder.html(htmlContent); // placeholder'ı doldur

                // Dynamic dropdowns ve toggles için init çağrıları
                btkModule.initDynamicAddressDropdowns('#btkServiceDetailsPlaceholder'); // Sadece bu bölüm için tetikle
                ...
                // Enjekte edilen HTML'deki toggle'ları yeniden başlat
    ...
    $(document).ready(function() {
        ...
        // Bu yüzden, küçük bir gecikme ekleyerek DOM manipülasyonunun tamamlanmasına izin veriyoruz.
        setTimeout(function() {
             if (typeof window.btkModuleLink !== 'undefined') {
                btkModule.init();
            }
        }, 100); // 100ms gecikme genellikle yeterlidir. Daha kısa tutulabilir (örn: 50ms)
    });
