...
$hook = array(
    ...
    'function' => 'InvoiceCreationPreEmailWa',
    'description' => 'Executes as an invoice is being created in the admin area before the email is being sent.<br>Client Related<br>First Name: {firstname}, Last Name: {lastname}, Email Address: {email}, Company Name: {companyname}, Credit Balance: {credit}, Country: {country}, Currency: {currency}<br>Invoice Related<br> Due Date: {duedate}, Total: {total}, Invoice ID: {invoiceid} , Date Created: {date}, Date Paid: {datepaid}, Sub Total: {subtotal}, Tax: {tax}, Tax Rate: {taxrate}, Invoice Status: {status}, Invoice Payment Method: {paymentmethod}, Invoice Payment Link: {invoice_payment_link}, Invoice Pdf Link: {invoice_file}',
    'type' => 'client',
    'extra' => '',
...
if(!function_exists('InvoiceCreationPreEmailWa')){
    function InvoiceCreationPreEmailWa($args){
        ...
        logModuleCall('wamessenger',__FUNCTION__,$args,$api);    
    }
}
